...
The native HTTP V1 integration test imports a real Git fixture, serves health, ref discovery, and a commit body through loopback TCP, verifies exact ref/object metadata and commit bytes, rejects request bodies and non-GET methods, and proves a `0.0.0.0` bind fails. It does not claim Git smart-HTTP compatibility, public-listener safety, or browser behaviour.
