...
`yeokcham-server` adds a separate read-only Yeokcham-native HTTP V1 boundary: `GET /`, `GET /storage`, `GET /integrity`, `GET /mirror`, `GET /commits/<sha1>`, `GET /trees/<sha1>`, `GET /v1/health`, `GET /v1/refs`, and `GET /v1/objects/<sha1>`. The browser renders repository/ref/HEAD metadata plus bounded commit/tree metadata, counts produced only after full local immutable-storage verification, an explicit successful integrity result, and redacted GitHub-mirror policy state; raw object and blob bodies remain native API data only. A failed integrity check returns a generic status without a partial inventory or repair action. The mirror page shows only policy direction plus rule/checkpoint counts, omits target/ref/object/timestamp data, and performs no GitHub transport or credential operation. The storage page reports that the V1 process has one local canonical provider and zero attached remote backends; it never discovers remote configuration, contacts a provider, or loads credentials. Ref names are exact hexadecimal bytes in JSON and HTML-escaped text or hexadecimal bytes in the browser; objects are reconstructed and Git-ID-verified before response. Commit pages show bounded metadata/message previews and linked parents/tree; tree pages validate binary entries, bound rendered entries/name previews, and link child trees only. It is deliberately not Git smart HTTP, so Git continues to use the local remote helper. Every endpoint requires one exact token loaded from an operator-managed private regular file: native clients use Bearer, while browser Basic uses username `yeokcham` and the same password token. The server compares the token in constant time and does not log it. The server accepts only a loopback numeric socket address, defaults to `127.0.0.1:0`, permits no request body or persistent connection, and bounds headers, body construction, deadlines, workers, and accepted-socket backlog. See [`docs/native-http-v1.md`](docs/native-http-v1.md), ADR-0092, ADR-0094, ADR-0095, ADR-0096, ADR-0097, and ADR-0098.
