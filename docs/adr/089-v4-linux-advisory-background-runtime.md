# ADR-089 — V4 Linux advisory background runtime
...
V4 already has two deliberately separate operations: Linux `watch` observes
...
second project format or a dependency of V4 recovery.

## Decision

V4 provides a Linux-only, per-repository managed runtime:
...
`start` detaches one process after verifying an existing V4 project. It holds
...
directory is `XDG_RUNTIME_DIR/yeokcham-v4/<root-hash>/`, where the hash is
...
current task, and bounded last result. It is not V4 project state, a package,
...
It calls the shared `Yeokcham_v4_sync` orchestration, which is also used by the
...
2. All runtime artifacts are private, bounded, disposable, and outside the
   canonical V4 object store and mutable project head. Deleting them cannot
