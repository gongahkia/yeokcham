# ADR-080 — V4 in-place restore journal
...
V4 can copy a retained checkpoint into an empty directory, but that does not
...
V4 is the only product model governed by this decision. Existing generic exact
...
Selected. V4 retains the current snapshot in project state, then appends
...
The canonical V4 restore journal is a six-field CBOR array:
...
Before `Prepared`, the safety snapshot is stored and published as a retained V4
...
The V4 project-state schema is unchanged. Journal schema version 1 has a golden
fixture. There is no migration from prior Yeokcham versions and no prior V4
