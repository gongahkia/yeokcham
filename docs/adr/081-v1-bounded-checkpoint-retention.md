# ADR-081 — V1 bounded checkpoint retention

- Status: Accepted
- Date: 2026-08-27
- Deciders: maintainers
- Supersedes: None
- Superseded by: ADR-085, for its pre-release V1 persistence-version and
  migration clauses only; ADR-090 for completed in-place restore retention

## Context and problem statement

V1 restore left every save on the checkpoint timeline and left published
...
open decision, or restore-safety. The object store has no delete API, and V1
...
- Keep pins in the one final V1 project-state schema.
...
Rejected. The V1 store cannot delete objects, and concurrent readers need the
...
The final `V1_project_state` version-1 schema includes the canonical pin array.
ADR-085 supersedes the earlier pre-release V1 decoder/migration proposal:
