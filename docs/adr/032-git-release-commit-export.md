...
M8-08 exports one immutable Yeokcham release as one independently inspectable Git
...
Yeokcham release or pretending a Git commit is Yeokcham history.
...
M8-08 accepts a visible, verified Yeokcham release and an absolute existing Git
...
name `refs/heads/yeokcham/release-<lowercase-release-id-hex>`.

Git commit metadata is fixed and not user identity: author and committer are
`Yeokcham Export <noreply@yeokcham.local>`; both timestamps are the release's
...
`Yeokcham release <lowercase-release-id-hex>\n`. This metadata describes an
...
structured unsupported-representation error before Git ref or Yeokcham mapping
...
object format, and exact commit ID. The Git ref and Yeokcham mapping binding are
...
- A destination ref cannot be overwritten by a different commit; Git objects
  may be unreferenced after a pre-ref failure and are not Yeokcham state.
- An exported Git commit is inspectable with normal Git tooling, but is not a
  Yeokcham release replacement and does not establish Git repository identity.
...
- Retry cannot advance or mutate a Yeokcham release, Git mapping, or target ref.
...
are external interchange artifacts, not Yeokcham canonical storage.
...
After acceptance, `yeokcham git export release --repository <absolute-git-directory>
