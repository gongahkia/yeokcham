# ADR-095 — V1 source-release verification
...
V1's identity, authority, package receipt, relay, and delivery rules concern a
...
check. It must not turn a Git signer into a V1 device, trust a public key by
first observation, create a release record, or silently publish anything.

## Decision

`tools/verify-v1-source-release.sh` verifies one candidate supplied entirely
by its caller:
...
1. A source-release signature is maintainer/consumer supply-chain evidence,
   never V1 device identity, authority, membership, package validity, relay
