# ADR-083 — V1 device identity, signatures, and verified receive
...
V1 currently has useful local change composition, but a device identifier is
...
define the smaller V1 source-control model. Reusing a cryptographic primitive
is appropriate; importing prior authority graphs, encrypted state, or transport
semantics is not.

## Decision

V1 uses one Ed25519 signing key per device. Its public key deterministically
...
Each V1 work record has a canonical signed envelope. Its signature binds the
...
Private keys are behind a V1 signer-provider interface. The first production
providers use macOS Keychain and Linux Secret Service. Private bytes are not
V1 state, immutable objects, package entries, diagnostics, or fixtures. The
test-only file provider requires the explicit
`YEOKCHAM_V1_TEST_SIGNER_DIRECTORY` environment variable. ADR-093 extends the
...
V1 state-head update. It never modifies a working tree, draft, or delivery.
...
8. A failed package never advances the V1 state head or mutates the working
   directory.

## Consequences

V1 gains explicit, inspectable authorship and offline collaboration without
