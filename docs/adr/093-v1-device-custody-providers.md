# ADR-093 — V1 local device-custody providers
...
key deliberately held by an SSH agent or a PKCS#11 token, where V1 must be able
...
V1 supports three local custody paths:
...
3. An explicit PKCS#11 profile selects an absolute module path, token label,
   binary key ID, and exact public key. V1 creates Ed25519 token keys with
...
2. Provider metadata and local custody profiles never enter V1 signed records,
...
7. SSH-agent forwarding is not a V1 trust mechanism. Operators should use a
   dedicated agent key and disable forwarding where that would expose signing
   authority to another host.

## Consequences

V1 can use a non-exportable PKCS#11 Ed25519 key for normal shared revisions,
...
Ed25519 support, so V1 detects unsupported mechanisms at use time rather than
...
non-exportability, exact SSH-agent selection, PKCS#11 signing for every V1
