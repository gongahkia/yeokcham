# ADR-094 — V1 external LSP semantic sidecars
...
canonical V1 answer to a decision: it can identify exact source provenance but
...
V1 may query one explicitly configured, already-installed Language Server
...
V1 writes. This limitation is displayed in command documentation rather than
...
4. Server-originated edit, configuration, command, and capability-changing
   requests cannot alter local V1 state or files through the adapter.
