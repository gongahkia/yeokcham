# ADR-091 — V1 local recoverable garbage collection
...
V1 checkpoint compaction removes unneeded checkpoint names from current project
...
recovery promise already represented by V1 state.
...
- the object selected by the sole `v1-project-state` mutable head;
...
in the current V1 project-state object; they do not create independent store
...
The collector does not retain an object merely because an old V1 state object
