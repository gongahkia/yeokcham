# ADR-096 — V1 macOS advisory watcher
...
V1's foreground `watch` command is an optional convenience around the exact
...
providing a trustworthy old/new path pair. V1 must therefore never convert an
...
2. No FSEvents path, event ID, callback output, debounce state, or native queue
   enters V1 project state, object storage, packages, relay data, authority,
...
None. The adapter has no durable record and changes no canonical V1 bytes.
