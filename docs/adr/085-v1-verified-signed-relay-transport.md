# ADR-085 — V1 verified signed relay transport

- Status: Accepted
- Date: 2026-08-28
- Implements: GitHub issue #246 (`V1-TRANSPORT-001`)

## Context

V1 already exchanges a complete canonical directory package and verifies it in
...
product-track peer protocols are retired with the V1-only cutover and are not a
compatibility target or implementation substrate.

## Current milestone and vertical slice

The current milestone is **V1 verified signed relay synchronization for
...
V1 repository ID, publisher device ID, publisher certificate ID, canonical
sorted parent publication IDs from that publisher's feed, and the SHA-256 ID of
one canonical V1 package manifest. Its enclosing record adds an Ed25519
...
A valid batch atomically publishes V1 state with its cursor, seen-publication
...
the V1 collaboration model. Test-only credential injection requires an
...
pre-release V1 schema variants are rejected and have no migration path. No
