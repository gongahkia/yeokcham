...
is Paengi selection state; no Git branch is created, inspected, or implied.
...
The final `.paengi/demo-v1-workspace-order-final` contains the same two ordered
revision IDs passed to `work reorder`. `.paengi/demo-v1-workspace-disabled`
...
ADR, `paengi` CLI behavior, materialisation, conflict, release, Git export,
