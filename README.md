...
`init` requires a destination path that does not exist. A failed import can leave unreachable immutable records in that fresh path; remove the failed destination before retrying. The initial CDC threshold is 4 KiB; `--chunked-blob-minimum <bytes>` is available for controlled storage-policy comparison, not as a benchmark-backed default recommendation. Import reads independent source objects with up to `min(logical CPUs, 8)` workers while retaining at most 64 MiB of verified bodies before serial publication; `--object-read-workers <1..8>` is available for controlled comparison.
