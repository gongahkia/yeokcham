...
`yeokcham-server token create <private-token-file>` creates a private random token file without printing its secret. `yeokcham-server --repository <yeokcham-repo> --auth-token-file <private-token-file>` then starts the read-only Yeokcham-native HTTP V1 service on an ephemeral `127.0.0.1` port and prints its address. `--bind 127.0.0.1:<port>` selects a known loopback port. Every endpoint requires the token: native clients use Bearer, while a normal browser can use Basic username `yeokcham` with the token as password to view repository refs, bounded commit/tree metadata, verified local-storage counts, and a read-only integrity result. It reports that V1 attaches no remote backend and performs no backend network or credential call. It also serves native health, ref discovery, and verified raw Git-object retrieval. It is not Git smart HTTP and accepts no public address. See [the V1 contract](docs/native-http-v1.md).
