...
- Task: replace the AFL encoding harness with bounded deterministic property tests (in progress).
...
- Tests: checked-in valid and malformed fixtures; bounded QCheck values and byte streams; explicit zero, boundary, near-limit, and malformed lengths; independent reproducible per-property seeds.
