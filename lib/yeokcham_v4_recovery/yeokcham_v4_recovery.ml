...
let error_to_string = function
  | Invalid_secret detail -> "invalid V4 recovery secret: " ^ detail
...
let verification_phrase certificate =
  let entropy =
    digest "yeokcham:v4:root-verification-phrase:1\000"
...
let encode_device device =
  let id = Trust.device_id device |> Yeokcham_v4_model.Device_id.to_string in
  let* id = text id in
