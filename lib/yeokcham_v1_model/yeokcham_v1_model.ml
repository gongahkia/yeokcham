...
let active_draft project =
  ...
  | Some draft -> draft
  | None -> invalid_arg "V1 project lacks its active draft"
...
let latest_revision (change : shared_change) =
  ...
  | revision :: _ -> revision
  | [] -> invalid_arg "V1 shared change lacks an initial revision"
...
let amend_active project revision =
  ...
  | Some change_id -> (
      match find_change project change_id with
      | None -> invalid_arg "active draft refers to a missing V1 shared change"
      | Some change -> (
...
let decision_id kind candidates =
  ...
  with
  | Ok id -> id
  | Error _ -> invalid_arg "generated V1 decision identifier is invalid"
  ...
  then Error Duplicate_draft
    ...
    then Error Delivery_includes_unknown_revision
      ...
      | Some active_change -> (
          ...
          | None ->
              invalid_arg "active draft refers to a missing V1 shared change"
          | Some change ->
