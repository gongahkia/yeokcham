...
let error_to_string = function
  ...
  | No_configured_relay_repository ->
      "V1 repair relay source requires a collaborative repository identity"
  | Bootstrap_error error -> Bootstrap.error_to_string error
  | Invalid_bootstrap_artifact { path; detail } ->
      Printf.sprintf "invalid V1 repair bootstrap artifact %s: %s" path detail
  | Missing_state_head -> "V1 repair target has no state head"
  | Invalid_backup { path; detail } ->
      Printf.sprintf "invalid V1 repair backup %s: %s" path detail

let store_of_target ~root =
  V1_store.open_repository ~root
  |> Result.map V1_store.underlying_store
  |> Result.map_error (fun error -> V1_store_error error)

let state_head store =
  let* reference =
    Store.read_ref store ~name:V1_store.state_head_name
...
let collaborative_repository ~root =
  ...
  let* loaded =
    ...
    |> Result.map_error (fun error -> V1_store_error error)
  in
  match loaded.V1_store.collaboration with
  | None -> Error No_configured_relay_repository
  | Some collaboration ->
      let repository = V1_store.membership collaboration |> Trust.repository in
      Ok repository
...
let apply_from_source ~root ~plan_id ~selection ~now ~matches_source
    ~read_candidate =
  let* target = store_of_target ~root in
  Store.with_lock target ~name:V1_store.state_head_name
