...
module Protocol = struct
  ...
  let make_source_file ~path ~language ~contents : source_file =
  ...
  let compiler_module_resolution (options : compiler_options) =
  ...
  let make_symbol ~qualified_name ~alias_qualified_name ~declaration_locations
      ~merged_declaration_count : symbol =
    {
      qualified_name;
      alias_qualified_name;
      declaration_locations;
      merged_declaration_count;
    }
  ...
  let make_declaration ~path ~declaration_kind ~declaration_span ~name_span
      ...
      ~overload_ordinal ~declaration_shape_digest ~signature_digest ~symbol :
      declaration =
    {
      path;
      declaration_kind;
  ...
  let make_diagnostic ~code ~category ~message ~path ~span : diagnostic =
    { code; category; message; path; span }

  let make_resolution_diagnostic ~containing_file ~module_name :
      resolution_diagnostic =
    { containing_file; module_name }
  ...
  let analysis_typescript_version (analysis : analysis) =
    analysis.typescript_version

  let analysis_parser_complete (analysis : analysis) = analysis.parser_complete

  let analysis_resolution_complete (analysis : analysis) =
    analysis.resolution_complete

  let analysis_semantic_complete (analysis : analysis) =
    analysis.semantic_complete
  ...
  let declaration_kind (declaration : declaration) =
    declaration.declaration_kind

  let declaration_span (declaration : declaration) =
    declaration.declaration_span

  let declaration_name_span (declaration : declaration) = declaration.name_span

  let declaration_syntactic_name (declaration : declaration) =
    declaration.syntactic_name

  let declaration_exported (declaration : declaration) = declaration.exported

  let declaration_shape_digest (declaration : declaration) =
    declaration.declaration_shape_digest
...
let default_configuration =
  {
    node = "node";
    adapter_path = "tools/yeokcham-typescript-adapter/adapter.mjs";
    timeout_ms = 5_000;
...
let unavailable_reason_to_string = function
  ...
  | Snapshot_error error -> Yeokcham_snapshot.error_to_string error
...
let make_replace_target ~path ~declaration_span ~expected_preimage
    ~declaration_kind ~declaration_shape_digest =
  {
    path;
    declaration_span;
    expected_preimage;
...
let replace_outcome_contents = function
  | Replaced { contents; _ } -> Some contents
  | Replace_conflict _ -> None

let replace_outcome_confidence = function
  | Replaced { confidence; _ } -> Some confidence
  | Replace_conflict _ -> None

let replace_outcome_fallback_used = function
  | Replaced { fallback_used; _ } -> Some fallback_used
  | Replace_conflict _ -> None

let replace_outcome_conflict_code = function
  | Replaced _ -> None
  | Replace_conflict { code; _ } -> Some code
...
let sha256_hex contents =
  Yeokcham_hash.Sha256.digest_string contents
  |> Yeokcham_hash.Sha256.to_raw_string |> hex

let language_to_string = function Protocol.Ts -> "ts" | Protocol.Tsx -> "tsx"

let compiler_options_json options =
  let fields =
    [ ("strict", string_of_bool (Protocol.compiler_strict options)) ]
  in
  let fields =
  ...
  in
  let fields =
    if paths = "" then fields else ("paths", "{" ^ paths ^ "}") :: fields
  in
  "{"
  ^ String.concat ","
...
let request_for_analysis ~snapshot_id ~root_files ~files ~compiler_options
    ~timeout_ms =
  let files =
    files
    |> List.map (fun (file : Protocol.source_file) ->
        ...
        ^ String.concat ","
            [
              ...
              Json.quote "language" ^ ":"
              ^ Json.quote
                  (language_to_string (Protocol.source_file_language file));
  ...
  request_json ~operation:"analyze"
    [
      ...
      ( "rootFiles",
        "[" ^ String.concat "," (List.map Json.quote root_files) ^ "]" );
      ("files", "[" ^ files ^ "]");
      ("compilerOptions", compiler_options_json compiler_options);
      ("timeoutMs", string_of_int timeout_ms);
    ]

let request_for_replace ~snapshot_id ~root_files ~files ~compiler_options
    ~timeout_ms ~target ~replacement =
  let files =
    files
    |> List.map (fun (file : Protocol.source_file) ->
        let file_path = Protocol.source_file_path file in
    ...
    |> String.concat ","
  in
  let target_span = target.declaration_span in
  request_json ~operation:"replace-node"
    [
      ("snapshotId", Json.quote snapshot_id);
      ( "rootFiles",
        "[" ^ String.concat "," (List.map Json.quote root_files) ^ "]" );
      ...
      ( "target",
        ...
        ^ String.concat ","
            [
              Json.quote "path" ^ ":" ^ Json.quote target.path;
              Json.quote "declarationStartByte"
              ^ ":"
              ^ string_of_int (Protocol.span_start_byte target_span);
              Json.quote "declarationEndByte"
              ^ ":"
              ^ string_of_int (Protocol.span_end_byte target_span);
              Json.quote "expectedPreimageHex"
              ^ ":"
              ^ Json.quote (hex target.expected_preimage);
              Json.quote "expectedPreimageSha256"
              ^ ":"
              ^ Json.quote (sha256_hex target.expected_preimage);
              Json.quote "declarationKind"
              ^ ":"
              ^ Json.quote target.declaration_kind;
              Json.quote "declarationShapeDigest"
              ^ ":"
              ^ Json.quote target.declaration_shape_digest;
            ]
        ^ "}" );
      ("replacementHex", Json.quote (hex replacement));
    ]
...
let request_descriptor request =
  try
    let path = Filename.temp_file "yeokcham-typescript-request-" ".json" in
      ...
      | Ok (request_path, stdin_read) ->
          Fun.protect
            ...
            (fun () ->
              ...
              let child =
                Unix.create_process_env node
                  [| node; configuration.adapter_path |]
...
let float_field name value =
  let* field = object_field name value in
  match Json.number field with
  | Some number -> Ok number
  | None -> Error ("response field " ^ name ^ " is not a number")
...
let decode_symbol value =
  ...
  let* alias_qualified_name =
    optional_string_field "aliasQualifiedName" value
  in
  let* declaration_locations =
    list_field "declarationLocations"
      (fun item ->
        match Json.string item with
        | Some string -> Ok string
        | None -> Error "symbol declaration location is not a string")
      value
  in
...
let decode_declaration value =
  ...
  let name_span =
    match
      ( Json.object_field "nameStartByte" value,
        Json.object_field "nameEndByte" value )
    with
    | None, None -> Ok None
    | Some _, Some _ ->
  ...
  let* parent_declaration_path =
    list_field "parentDeclarationPath"
      (fun item ->
        match Json.string item with
        | Some string -> Ok string
        | None -> Error "parent declaration path item is not a string")
      value
  in
  ...
  let symbol =
    ...
    | Some symbol -> (
        ...
        | Some (Json.Bool true) ->
            decode_symbol symbol |> Result.map Option.some
        | Some (Json.Bool false) -> Ok None
        | Some
            ( Json.Null | Json.Number _ | Json.String _ | Json.Array _
            | Json.Object _ )
        | None ->
...
let decode_diagnostic value =
  ...
  let span =
    match
      (Json.object_field "startByte" value, Json.object_field "endByte" value)
    with
    | None, None -> Ok None
    | Some _, Some _ ->
        decode_span "startByte" "endByte" value |> Result.map Option.some
...
let decode_analysis value =
  ...
  let* resolution_diagnostics =
    list_field "resolutionDiagnostics" decode_resolution_diagnostic value
  in
  let* type_checker_diagnostics =
    list_field "typeCheckerDiagnostics" decode_diagnostic value
  in
...
let decode_hex value =
  if String.length value mod 2 <> 0 then Error "hex response has odd length"
  else
    let bytes = Bytes.create (String.length value / 2) in
    let rec fill index =
      if index = Bytes.length bytes then Ok (Bytes.unsafe_to_string bytes)
      else
        match byte_of_hex_pair value.[2 * index] value.[(2 * index) + 1] with
        | None -> Error "hex response contains a non-hex byte"
        | Some byte ->
...
let decode_replace_success value =
  let* path = string_field "path" value in
  let* contents_hex = string_field "newFileContentsHex" value in
  let* contents = decode_hex contents_hex in
  let* parser_complete = bool_field "parserComplete" value in
  let* resolution_complete = bool_field "resolutionComplete" value in
  let* type_resolution_complete = bool_field "typeResolutionComplete" value in
  let* evidence =
    list_field "evidence"
      (fun item ->
        match Json.string item with
        | Some value -> Ok value
        | None -> Error "replace evidence item is not a string")
      value
  in
  let* confidence = string_field "confidence" value in
  let* fallback_used = bool_field "fallbackUsed" value in
  Ok
    (Replaced
       {
         path;
...
let decode_replace_response output =
  ...
  if response_version <> Protocol.version then
    ...
    | "ok" ->
        ...
        decode_replace_success result
    | "conflict" ->
        let* conflict = object_field "conflict" response in
        let* code = string_field "code" conflict in
        let* message = string_field "message" conflict in
        Ok (Replace_conflict { code; message })
    | "error" ->
...
let run configuration request decode =
  if not (Sys.file_exists configuration.adapter_path) then
...
let analyze_files configuration ~snapshot_id ~root_files ~files
    ~compiler_options =
  if
    (not
       (List.for_all
  ...
  then
    Unavailable
      (Adapter_error
         {
           code = "invalid-path";
           message = "semantic input contains an unsafe project-relative path";
         })
  else
    let request =
      request_for_analysis ~snapshot_id ~root_files ~files ~compiler_options
        ~timeout_ms:configuration.timeout_ms
    in
    run configuration request (decode_response decode_analysis)

let replace_node_files configuration ~snapshot_id ~root_files ~files
    ~compiler_options ~target ~replacement =
  if
    (not (safe_relative_path target.path))
    || (not
          (List.for_all
             (fun (file : Protocol.source_file) ->
               safe_relative_path (Protocol.source_file_path file))
      ...
      (Adapter_error
         {
           code = "invalid-path";
           message =
             "semantic replacement contains an unsafe project-relative path";
         })
  else
    let request =
      request_for_replace ~snapshot_id ~root_files ~files ~compiler_options
        ~timeout_ms:configuration.timeout_ms ~target ~replacement
    in
    run configuration request decode_replace_response

let language_for_path path =
  if String.ends_with ~suffix:".tsx" path then Some Protocol.Tsx
  else if
    String.ends_with ~suffix:".ts" path || String.ends_with ~suffix:".d.ts" path
  then Some Protocol.Ts
  else None

let analyze_snapshot configuration ~store ~snapshot ~compiler_options =
  let rec collect path tree_id =
    let* tree = Yeokcham_snapshot.Tree.load store tree_id in
    Yeokcham_snapshot.Tree.entries tree
    |> List.fold_left
         (fun accumulated (name, entry) ->
           let* collected = accumulated in
           let path = path @ [ name ] in
           match entry with
           | Yeokcham_snapshot.Tree.Directory tree_id ->
               let* nested = collect path tree_id in
               Ok (List.rev_append nested collected)
           | Yeokcham_snapshot.Tree.File
               { mode = Yeokcham_snapshot.Symlink; content = _ } ->
               Ok collected
           | Yeokcham_snapshot.Tree.File
               {
                 mode = Yeokcham_snapshot.Regular | Yeokcham_snapshot.Executable;
                 content;
               } -> (
         ...
         (Ok [])
  in
  match Yeokcham_snapshot.Snapshot.load store snapshot with
  | Error error -> Unavailable (Snapshot_error error)
  | Ok snapshot_model -> (
      match collect [] (Yeokcham_snapshot.Snapshot.root snapshot_model) with
      | Error error -> Unavailable (Snapshot_error error)
      | Ok files ->
          let files =
            List.sort
          ...
          let snapshot_id =
            Yeokcham_snapshot.Snapshot.stored_object_id snapshot
              ...
              (Adapter_error
                 {
                   ...
                   message =
                     "verified snapshot contains no TypeScript source files";
                 })
          else
            analyze_files configuration ~snapshot_id ~root_files ~files
              ~compiler_options)
