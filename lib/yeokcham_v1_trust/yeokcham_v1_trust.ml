...
module Record = Yeokcham_v1_record
module Encoding = Yeokcham_encoding

let ( let* ) = Result.bind
let algorithm = "ed25519"
let certificate_id_domain = "yeokcham:v1:certificate-id:1\000"
let certificate_signature_domain = "yeokcham:v1:certificate-signature:1\000"
let revision_signature_domain = "yeokcham:v1:revision-signature:1\000"
let authority_epoch_id_domain = "yeokcham:v1:authority-epoch-id:1\000"

let authority_epoch_signature_domain =
  "yeokcham:v1:authority-epoch-signature:1\000"

let authorization_signature_domain = "yeokcham:v1:authorization:1\000"
let adoption_signature_domain = "yeokcham:v1:adoption:1\000"
...
(* A capability is an opaque local signing operation.  In particular, an
   external token or agent never needs to reveal private material to the V1
   process. *)
...
let error_to_string = function
  | Invalid_public_key length ->
      Printf.sprintf "V1 Ed25519 public key must contain 32 bytes, got %d"
        length
  | Invalid_signature length ->
      Printf.sprintf "V1 Ed25519 signature must contain 64 bytes, got %d" length
  | Invalid_private_key -> "V1 signing capability does not match its device"
  | Entropy_failure -> "V1 OS CSPRNG is unavailable"
  | Unsupported_algorithm value ->
      "unsupported V1 signature algorithm: " ^ value
  | Unsupported_features features ->
      Printf.sprintf "unsupported V1 mandatory features: %Ld" features
  | Invalid_record detail -> "invalid V1 signed record: " ^ detail
  | Noncanonical_record -> "V1 signed record is not canonically encoded"
  | Identity_mismatch detail -> "V1 signed identity mismatch: " ^ detail
  | Signature_verification_failed -> "V1 Ed25519 signature is invalid"
  | Duplicate_certificate -> "duplicate V1 device certificate"
  | Unknown_issuer_certificate -> "V1 certificate issuer is unknown"
  | Unauthorized_issuer -> "V1 certificate issuer is not an administrator"
  | Invalid_root_certificate -> "V1 root certificate must be self-signed admin"
  | Cross_repository_certificate ->
      "V1 certificate belongs to another repository"
  | Duplicate_device -> "V1 device has more than one certificate"
  | Unknown_author_certificate -> "V1 revision author certificate is unknown"
  | Revision_author_mismatch ->
      "V1 revision author does not match its author certificate"
  | Unknown_epoch -> "V1 authority epoch is unknown"
  | Invalid_epoch detail -> "invalid V1 authority epoch: " ^ detail
  | Duplicate_epoch -> "duplicate V1 authority epoch"
  | Authority_fork ->
      "V1 authority fork requires an explicit authority selection or \
       reconciliation"
  | Revoked_device -> "V1 device is revoked in this authority epoch"
  | Unauthorized_epoch_issuer ->
      "V1 authority epoch issuer is not an active administrator in every parent"
  | Invalid_recovery_authority -> "invalid V1 recovery authority"
  | Unknown_authorization -> "V1 one-time authorization is unknown"
  | Authorization_mismatch -> "V1 authorization does not match this revision"
  | Duplicate_authorization -> "V1 authorization was already consumed"
  | Signing_failed detail -> "V1 signer failed: " ^ detail
  | Private_key_unavailable ->
      "V1 signer is non-exportable and cannot provide private key bytes"
...
let device_id_of_public_key public_key =
  if String.length public_key <> 32 then
    ...
    Repository_id.hex (digest "yeokcham:v1:device-id:1\000" public_key)
...
let role_of_code = function
  ...
  | _ -> Error (Invalid_record "unknown V1 certificate role")
...
let decode_certificate encoded =
  ...
  in
  let* values = exact_array "V1 certificate" 2 value in
  match values with
  | [ unsigned; signature ] -> (
      let* fields = exact_array "V1 certificate unsigned body" 8 unsigned in
...
let sign_revision _ ~certificate:_ _ _ =
  Error (Invalid_epoch "V1 signed revisions require an authority epoch")

let sign_resolution _ ~certificate:_ _ ~decision:_ _ =
  Error (Invalid_epoch "V1 signed resolutions require an authority epoch")
...
let encode_signed_revision signed =
  let epoch =
    ...
    | Some epoch -> epoch
    | None -> invalid_arg "V1 signed revisions require an authority epoch"
  in
  let unsigned =
...
let decode_signed_revision encoded =
  let* value =
    ...
    |> Result.map_error (fun error ->
        Invalid_record (Encoding.decode_error_to_string error))
  in
  let* values = exact_array "V1 signed revision" 2 value in
  match values with
  | [ unsigned; signature ] -> (
      ...
      let* fields =
        ...
        | Encoding.Array _ ->
            Error
              (Invalid_record
                 "V1 revision unsigned body has the wrong field count")
        | Encoding.Integer _ | Encoding.Bytes _ | Encoding.Text _
        | Encoding.Map _ | Encoding.Bool _ | Encoding.Null ->
            Error (Invalid_record "V1 revision unsigned body must be an array")
      in
      match fields with
      | [ version; repository; certificate; revision ] ->
          ...
          Error
            (Invalid_record
               "authority-less signed revision encoding was retired before V1 \
                release")
      | [ version; repository; certificate; epoch; revision ] ->
          ...
          Error
            (Invalid_record
               "pre-release signed revision encoding was retired before V1 \
                release")
...
let verify_signed_revision_crypto membership signed =
  ...
  then Error Cross_repository_certificate
    ...
    let* epoch =
      ...
      | Some epoch -> Ok epoch
      | None ->
          Error (Invalid_epoch "V1 signed revision has no authority epoch")
    in
    match
...
let verify_signed_revision membership signed =
  let _ = membership in
  let _ = signed in
  Error (Invalid_epoch "V1 signed revisions require authority verification")
...
let decode_epoch encoded =
  let* value =
    ...
    |> Result.map_error (fun error ->
        Invalid_record (Encoding.decode_error_to_string error))
  in
  let* values = exact_array "V1 authority epoch" 2 value in
  match values with
  | [ unsigned; signature ] -> (
      let* signature = bytes_field "authority epoch signature" signature in
      let* fields = exact_array "V1 authority epoch unsigned body" 9 unsigned in
      match fields with
      | [
       version;
       repository;
...
let decode_authorization encoded =
  let* value =
    ...
    |> Result.map_error (fun error ->
        Invalid_record (Encoding.decode_error_to_string error))
  in
  let* values = exact_array "V1 authorization" 2 value in
  match values with
  | [ unsigned; signature ] -> (
      let* signature = bytes_field "authorization signature" signature in
      let* fields = exact_array "V1 authorization unsigned body" 7 unsigned in
      match fields with
...
let signed_revision_digest signed =
  Repository_id.hex
    (digest "yeokcham:v1:signed-revision-digest:1\000"
...
let decode_adoption encoded =
  let* value =
    ...
    |> Result.map_error (fun error ->
        Invalid_record (Encoding.decode_error_to_string error))
  in
  let* values = exact_array "V1 adoption" 2 value in
  match values with
  | [ unsigned; signature ] -> (
      let* signature = bytes_field "adoption signature" signature in
      let* fields = exact_array "V1 adoption unsigned body" 6 unsigned in
      match fields with
