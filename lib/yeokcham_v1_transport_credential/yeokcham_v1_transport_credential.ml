...
let schema = "v1-relay-bearer-1"
let test_enabled_environment = "YEOKCHAM_V1_TEST_TRANSPORT"
let test_token_environment = "YEOKCHAM_V1_TEST_TRANSPORT_TOKEN"
let max_output = 8192

let error_to_string = function
  | Unavailable -> "V1 transport credential store is unavailable"
  | Missing -> "V1 transport credential is absent"
  | Rejected -> "V1 transport credential store rejected the request"
  | Invalid -> "V1 transport credential is invalid"
...
let save ~remote ~token =
  if not (valid_token token) then Error Invalid
    ...
    | None -> (
        match
          command
            ("store" :: "--label=Yeokcham V1 relay credential"
           :: attributes remote)
            token
        with
        | Ok (0, _) -> Ok ()
