...
let error_to_string = function
  ...
  | Invalid_extension extension ->
      "invalid semantic file extension: " ^ extension
...
let valid_scope = function
  ...
  | Extensions extensions ->
      extensions <> []
      && sorted_unique String.compare extensions
      && List.for_all valid_extension extensions
  | Path_globs globs ->
      globs <> []
      && sorted_unique String.compare globs
      && List.for_all valid_glob globs
...
let encode servers =
  ...
  array [ Encoding.integer schema_version; servers ]
...
let array_values name = function
  ...
  | Encoding.Bool _ | Encoding.Null ->
      Error (Encoding_error (name ^ " must be an array"))
...
let text_field name = function
  ...
  | Encoding.Bool _ | Encoding.Null ->
      Error (Encoding_error (name ^ " must be text"))
...
let bool_field name = function
  ...
  | Encoding.Map _ | Encoding.Null ->
      Error (Encoding_error (name ^ " must be boolean"))

let integer_field name = function
  | Encoding.Integer value -> Ok value
  | Encoding.Bytes _ | Encoding.Text _ | Encoding.Array _ | Encoding.Map _
  | Encoding.Bool _ | Encoding.Null ->
      Error (Encoding_error (name ^ " must be integer"))
...
let decode_server value =
  ...
  | [ name; program; arguments; enabled; match_scope; sensitivity ] ->
      ...
      let* arguments =
        decode_string_array "semantic server arguments" arguments
      in
...
let decode bytes =
  ...
  | _ ->
      Error (Encoding_error "semantic server config has the wrong field count")
      ...
      (Io_error
         {
           path = directory;
           operation = "open";
           message = "repository metadata directory is absent";
         })
  else
    let temporary =
      Filename.temp_file ~temp_dir:directory ".semantic-lsp-" ".tmp"
    in
    try
      let descriptor =
...
let add ~root ~server =
  if not (valid_name server.name) then Error (Invalid_name server.name)
  else if not (valid_program server.program) then
    Error (Invalid_program server.program)
    ...
    if
      List.exists
        (fun existing -> String.equal existing.name server.name)
        servers
    then Error (Duplicate_server server.name)
    else
      write_config ~root
        (List.sort
...
let update ~root ~name change =
  let* servers = read_config ~root in
  if not (List.exists (fun server -> String.equal server.name name) servers)
  then Error (Unknown_server name)
  else
    write_config ~root
      (List.map
         (fun server ->
           if String.equal server.name name then change server else server)
         servers)
...
let remove ~root ~name =
  ...
  then Error (Unknown_server name)
    ...
    write_config ~root
      (List.filter (fun server -> not (String.equal server.name name)) servers)

let set_enabled ~root ~name ~enabled =
  update ~root ~name (fun server -> { server with enabled })

let configure ~root ~name ~match_scope ~overlap_sensitivity =
  if not (valid_scope match_scope) then
    Error (Invalid_match_scope "invalid scope")
  else
    update ~root ~name (fun server ->
        { server with match_scope; overlap_sensitivity })
...
let matches_path server candidate =
  if not (valid_relative_path candidate) then false
    ...
    | Extensions extensions ->
        List.exists
          (fun extension -> String.ends_with ~suffix:extension candidate)
          extensions
    | Path_globs globs ->
        List.exists
          (fun glob ->
