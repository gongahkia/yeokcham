...
let is_temporary name =
  ...
  if String.length name <= 1 + 64 + String.length marker || name.[0] <> '.' then
    false
  else
...
let read_regular record_path =
  ...
  then Error (Invalid_record_path record_path)
  else
    try
          ...
          let rec read offset =
            if offset = stat.Unix.st_size then Ok ()
              ...
              try
                ...
                in
                if count = 0 then Error (Invalid_record_path record_path)
                else read (offset + count)
              with Unix.Unix_error (error, _, _) ->
                Error (io_error ~operation:"read" ~path:record_path error)
          in
          let* () = read 0 in
    ...
    with Unix.Unix_error (error, _, _) ->
      Error (io_error ~operation:"open" ~path:record_path error)
...
let write_all descriptor record_path bytes =
  let rec write offset =
    if offset = Bytes.length bytes then Ok ()
      ...
      with Unix.Unix_error (error, _, _) ->
        Error (io_error ~operation:"write" ~path:record_path error)
  in
  write 0
...
let cleanup directory temporary =
  ...
  | Unix.Unix_error (error, _, _) ->
      Error (io_error ~operation:"unlink" ~path:temporary error)
...
let publish ~directory ~final ~bytes =
  ...
  | Ok None ->
      ...
      let* actual = read_regular final in
      if String.equal actual bytes then Ok publication
      else Error (Record_collision final)

let write ~root ~authority transition =
  let bytes = Epoch.encode transition in
  let* checked =
    Epoch.decode ~authority bytes
  ...
  publish ~directory:store_directory
    ~final:(path ~root (Epoch.id checked))
    ~bytes
  ...
  let rec read records = function
    ...
    | name :: rest ->
        ...
        let* record =
          ...
          |> Result.map_error (fun error -> Epoch_error error)
        in
        if String.equal name (filename (Epoch.id record)) then
          read (record :: records) rest
        else Error (Invalid_record_path record_path)
  in
  read [] names
...
let load_current ~runtime ~root ~authority ~state_key ~initial_state =
  let* () = check_v2_root root in
  let* records =
    match lstat (directory ~root) with
    | Ok None -> Ok []
    | Ok (Some _) -> read_all ~root ~authority
    | Error error -> Error error
  in
