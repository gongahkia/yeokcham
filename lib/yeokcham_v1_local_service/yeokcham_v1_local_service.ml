...
module Restore_proof = Yeokcham_v1_restore_proof
...
let error_to_string = function
  ...
  | Unsigned_project ->
      "this V1 project has no signed collaboration state; initialize a signed \
       project"
...
let with_outbound_directory ~root run =
  try
    let destination = Filename.temp_file "yeokcham-v1-outbound-" ".tmp" in
