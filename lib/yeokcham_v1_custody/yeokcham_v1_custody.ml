...
let error_to_string = function
  | Profile_missing -> "V1 local custody profile is missing"
  | Profile_exists -> "V1 local custody profile already exists"
  | Invalid_profile detail -> "invalid V1 local custody profile: " ^ detail
  | Io_error { path; operation; message } ->
      Printf.sprintf "V1 custody %s %s: %s" operation path message
  | Ssh_agent_unavailable detail -> "V1 SSH agent is unavailable: " ^ detail
  | Ssh_agent_key_missing -> "configured V1 SSH agent key is unavailable"
  | Ssh_agent_protocol detail -> "invalid SSH agent response: " ^ detail
  | Pkcs11_unavailable detail -> "V1 PKCS#11 provider is unavailable: " ^ detail
  | Pkcs11_key_missing -> "configured V1 PKCS#11 key is unavailable"
  | Pkcs11_key_ambiguous ->
      "configured V1 PKCS#11 key selector matches more than one key"
  | Pkcs11_locked -> "V1 PKCS#11 token is locked or denied signing"
  | Pkcs11_unsupported detail -> "unsupported V1 PKCS#11 capability: " ^ detail
