...
enum { V1_OK = 0, V1_UNAVAILABLE = 1, V1_KEY_MISSING = 2, V1_LOCKED = 3,
       V1_UNSUPPORTED = 4, V1_INVALID = 5, V1_KEY_AMBIGUOUS = 6,
       V1_NOT_NONEXTRACTABLE = 7 };
...
static int status_for(CK_RV rv) {
  if (rv == CKR_PIN_INCORRECT || rv == CKR_PIN_LOCKED || rv == CKR_USER_NOT_LOGGED_IN)
    return V1_LOCKED;
  if (rv == CKR_MECHANISM_INVALID || rv == CKR_KEY_TYPE_INCONSISTENT) return V1_UNSUPPORTED;
  return V1_UNAVAILABLE;
}
...
static int open_module(const char *path, struct ctx *ctx) {
  ...
  if (ctx->handle == NULL) return V1_UNAVAILABLE;
  get_list = (C_GetFunctionListFn)dlsym(ctx->handle, "C_GetFunctionList");
  if (get_list == NULL || get_list(&ctx->f) != CKR_OK || ctx->f == NULL) return V1_UNAVAILABLE;
...
static int selected_slot(struct ctx *ctx, const char *label, CK_SLOT_ID *out) {
  ...
  rv = list(1, NULL, &count); if (rv != CKR_OK || count == 0 || count > 1024) return V1_UNAVAILABLE;
  slots = calloc(count, sizeof(*slots)); if (slots == NULL) return V1_UNAVAILABLE;
  rv = list(1, slots, &count); if (rv != CKR_OK) { free(slots); return V1_UNAVAILABLE; }
  for (i = 0; i < count; ++i) {
    ...
    if (strcmp(token_label, label) == 0) { *out = slots[i]; free(slots); return V1_OK; }
  }
  free(slots); return V1_KEY_MISSING;
}
...
static int open_session(struct ctx *ctx, const char *label, const char *pin) {
  ...
  status = selected_slot(ctx, label, &slot); if (status != V1_OK) return status;
  ...
  ctx->session_open = 1;
  if (pin == NULL) return V1_OK;
  rv = login(ctx->session, CKU_USER, (CK_BYTE *)pin, (CK_ULONG)strlen(pin));
  if (rv == CKR_OK || rv == CKR_USER_ALREADY_LOGGED_IN) { ctx->logged_in = (rv == CKR_OK); return V1_OK; }
...
static int find_key(struct ctx *ctx, CK_ULONG class_, const char *key_id, size_t key_id_len, CK_OBJECT_HANDLE *out) {
  ...
  if (rv != CKR_OK) return status_for(rv);
  if (count == 0) return V1_KEY_MISSING;
  if (count != 1) return V1_KEY_AMBIGUOUS;
  *out = found[0]; return V1_OK;
}
...
static int public_key_for(struct ctx *ctx, const char *key_id, size_t key_id_len, char raw[32]) {
  ...
  status = find_key(ctx, CKO_PUBLIC_KEY, key_id, key_id_len, &object); if (status != V1_OK) return status;
  ...
  point = malloc(attr.ulValueLen); if (point == NULL) return V1_UNAVAILABLE;
  ...
  else { free(point); return V1_INVALID; }
  free(point); return V1_OK;
}
...
static int private_key_is_nonextractable(struct ctx *ctx, const char *key_id, size_t key_id_len) {
  ...
  if (rv != CKR_OK) return status_for(rv);
  return sensitive && !extractable ? V1_OK : V1_NOT_NONEXTRACTABLE;
}

CAMLprim value caml_yeokcham_v1_pkcs11_public(value module_path, value token_label, value key_id) {
  ...
  if (status == V1_OK) status = open_session(&ctx, String_val(token_label), NULL);
  if (status == V1_OK) status = public_key_for(&ctx, String_val(key_id), caml_string_length(key_id), raw);
  cleanup(&ctx); CAMLreturn(result_for(status, status == V1_OK ? raw : NULL, 32));
}

CAMLprim value caml_yeokcham_v1_pkcs11_sign(value module_path, value token_label, value key_id, value pin, value data) {
  ...
  status = open_module(String_val(module_path), &ctx);
  if (status == V1_OK) status = open_session(&ctx, String_val(token_label), String_val(pin));
  if (status == V1_OK) status = find_key(&ctx, CKO_PRIVATE_KEY, String_val(key_id), caml_string_length(key_id), &key);
  if (status == V1_OK) {
    ...
    if (rv != CKR_OK) status = status_for(rv);
           ...
           if (rv != CKR_OK || length != 64) status = (rv == CKR_OK ? V1_INVALID : status_for(rv));
           else { out = malloc(length); if (out == NULL) status = V1_UNAVAILABLE;
                  else { rv = sign(ctx.session, (CK_BYTE *)String_val(data), caml_string_length(data), out, &length);
                         if (rv != CKR_OK || length != 64) status = (rv == CKR_OK ? V1_INVALID : status_for(rv)); } } }
  }
  cleanup(&ctx); { value result = result_for(status, status == V1_OK ? (const char *)out : NULL, 64); free(out); CAMLreturn(result); }
}

CAMLprim value caml_yeokcham_v1_pkcs11_create(value module_path, value token_label, value key_id, value key_label, value pin) {
