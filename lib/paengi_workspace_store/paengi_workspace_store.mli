...
val store_workspace :
  Paengi_store.repository -> workspace -> (Paengi_store.Stored_object_id.t, error) result
val load_workspace :
  Paengi_store.repository -> Paengi_store.Stored_object_id.t -> (workspace, error) result
...
val revision_resolved_order : workspace_revision -> Paengi_id.Capsule_revision_id.t list
...
val decode_revision_payload : Paengi_encoding.t -> (workspace_revision, error) result
...
val store_conflict :
  Paengi_store.repository -> conflict -> (Paengi_store.Stored_object_id.t, error) result
val load_conflict :
  Paengi_store.repository -> Paengi_store.Stored_object_id.t -> (conflict, error) result
...
val resolution_workspace_revision : resolution -> Paengi_id.Workspace_revision_id.t
...
val store_resolution :
  Paengi_store.repository -> resolution -> (Paengi_store.Stored_object_id.t, error) result
val load_resolution :
  Paengi_store.repository -> Paengi_store.Stored_object_id.t -> (resolution, error) result
...
val attempt_workspace_revision : workspace_attempt -> Paengi_id.Workspace_revision_id.t
...
val attempt_starting_checkpoint : workspace_attempt -> Paengi_scratch.Checkpoint_id.t
...
val attempt_resulting_snapshot : workspace_attempt -> Paengi_snapshot.Snapshot.id
...
val decode_attempt_payload : Paengi_encoding.t -> (workspace_attempt, error) result
val store_attempt :
  Paengi_store.repository ->
  workspace_attempt ->
  (Paengi_store.Stored_object_id.t, error) result
...
val make_current_ref :
  ...
  latest_attempt:(Paengi_id.Workspace_attempt_id.t * Paengi_store.Stored_object_id.t) option ->
...
val current_latest_attempt :
  current_ref -> (Paengi_id.Workspace_attempt_id.t * Paengi_store.Stored_object_id.t) option
...
module Durable : sig
  ...
  val read_current :
    Paengi_store.repository -> Paengi_id.Workspace_id.t -> (resolved, error) result
  ...
  val show_conflict :
    Paengi_store.repository -> Paengi_id.Conflict_id.t -> (conflict, error) result
