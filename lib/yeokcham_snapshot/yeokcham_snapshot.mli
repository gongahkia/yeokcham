...
type error =
  ...
  | Envelope_creation_error of Yeokcham_envelope.creation_error
...
module Content : sig
  ...
  val of_stored_object_id : Yeokcham_store.Stored_object_id.t -> id
  val stored_object_id : id -> Yeokcham_store.Stored_object_id.t
  val equal_id : id -> id -> bool
  val identity_of_bytes : string -> identity
  val identity_to_raw_bytes : identity -> string
  val store : Yeokcham_store.repository -> string -> (id, error) result
  val load : Yeokcham_store.repository -> id -> (string, error) result
end
...
module Manifest : sig
  ...
  val store_bytes : Yeokcham_store.repository -> string -> (id, error) result
  val load : Yeokcham_store.repository -> id -> (t, error) result
end
...
module Tree : sig
  ...
  val store : Yeokcham_store.repository -> t -> (id, error) result
  val load : Yeokcham_store.repository -> id -> (t, error) result
end
...
module Materialize : sig
  ...
  val plan :
    Yeokcham_store.repository -> Snapshot.t -> (action list, error) result
