(** Local create-only storage for canonical [repair-plan-v1] bytes.
    ...
    Repair plans are operator-local approval records. They are never V1 objects,
...
val append :
  root:string -> Yeokcham_v1_health.repair_plan -> (unit, error) result
(** Creates only a missing matching canonical plan. A divergent existing file
    remains in place and returns [Plan_collision]. *)

val find :
  root:string -> id:string -> (Yeokcham_v1_health.repair_plan, error) result

val scan : root:string -> (Yeokcham_v1_health.repair_plan list, error) result
