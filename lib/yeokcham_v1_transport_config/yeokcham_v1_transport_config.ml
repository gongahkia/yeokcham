...
let error_to_string = function
  ...
  | Duplicate_remote name -> "V1 transport remote already exists: " ^ name
  | Unknown_remote name -> "unknown V1 transport remote: " ^ name
  | Io_error { path; operation; message } ->
      Printf.sprintf "V1 transport config %s %s: %s" operation path message
  | Encoding_error detail -> "invalid V1 transport config: " ^ detail
...
let encode remotes =
  ...
  array [ Encoding.integer schema_version; remotes ]
...
let integer_field name = function
  ...
  | Encoding.Bool _ | Encoding.Null ->
      Error (Encoding_error (name ^ " must be an integer"))
...
let decode_remote value =
  ...
  | [ name; url ] ->
      ...
      let* () =
        ...
        |> Result.map_error (fun error ->
            Encoding_error (Transport.error_to_string error))
        |> Result.map (fun _ -> ())
      in
...
let decode bytes =
  ...
  | [ version; remotes ] ->
      ...
      if not (Int64.equal version schema_version) then
        Error (Encoding_error "unsupported config version")
      else
        ...
        let unique =
          ...
          = List.length
              (List.sort_uniq
                 ...
                 sorted)
        in
        if (not unique) || remotes <> sorted then
          Error (Encoding_error "remotes must be sorted and unique")
        else
          let* canonical = encode remotes in
          if String.equal canonical bytes then Ok remotes
          else Error (Encoding_error "config is not canonical")
...
let write_config ~root remotes =
  ...
  if not (Sys.file_exists directory) then
    ...
    let temporary =
      Filename.temp_file ~temp_dir:directory ".transport-remotes-" ".tmp"
    in
    try
      let descriptor =
...
let remove ~root ~name =
  let* remotes = read_config ~root in
  if not (List.exists (fun remote -> String.equal remote.name name) remotes)
  then Error (Unknown_remote name)
  else
    write_config ~root
      (List.filter (fun remote -> not (String.equal remote.name name)) remotes)
