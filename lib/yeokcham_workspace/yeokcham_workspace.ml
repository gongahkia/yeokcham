...
let entry_equal left right =
  ...
  | Yeokcham_scratch.Directory, Yeokcham_scratch.Directory -> true
  | Yeokcham_scratch.File left, Yeokcham_scratch.File right ->
      left.mode = right.mode
      && Yeokcham_snapshot.Content.equal_id left.content right.content
  | Yeokcham_scratch.Directory, Yeokcham_scratch.File _
  | Yeokcham_scratch.File _, Yeokcham_scratch.Directory ->
      false
...
let operation_current state (operation : Capsule.operation) =
  match operation with
  | Capsule.Exact_file_transition transition ->
      Yeokcham_scratch.State.find state transition.Capsule.transition_path
  | Capsule.Text_edit edit ->
      Yeokcham_scratch.State.find state edit.Capsule.edit_path
  | Capsule.Move { source; _ } -> Yeokcham_scratch.State.find state source
  | Capsule.Mode_change { path; _ } -> Yeokcham_scratch.State.find state path
...
let operation_already_satisfied state (operation : Capsule.operation) =
  ...
  | Capsule.Move { source; destination; prior } ->
      Option.is_none (Yeokcham_scratch.State.find state source)
      && Option.equal entry_equal
           (Yeokcham_scratch.State.find state destination)
           (Some prior)
  | Capsule.Mode_change { path; replacement; _ } -> (
      match Yeokcham_scratch.State.find state path with
      | Some (Yeokcham_scratch.File file) -> file.mode = replacement
      | Some Yeokcham_scratch.Directory | None -> false)
...
let apply_exact_transition state (transition : Capsule.exact_file_transition) =
  ...
  then Error "entry precondition failed"
    ...
    let operations =
      ...
      | None, Some entry ->
          [
            Yeokcham_scratch.Create
              { path = transition.Capsule.transition_path; entry };
          ]
      | Some prior, None ->
          [
            Yeokcham_scratch.Delete
    ...
    in
    Yeokcham_scratch.State.apply state operations
    |> Result.map_error Yeokcham_scratch.error_to_string
...
let apply_operation state (operation : Capsule.operation) =
  ...
  | Capsule.Move { source; destination; prior } ->
      Yeokcham_scratch.State.apply state
        [ Yeokcham_scratch.Move { source; destination; prior } ]
      |> Result.map_error Yeokcham_scratch.error_to_string
  | Capsule.Mode_change { path; expected; replacement } ->
      Yeokcham_scratch.State.apply state
        [ Yeokcham_scratch.Change_mode { path; expected; replacement } ]
      |> Result.map_error Yeokcham_scratch.error_to_string
