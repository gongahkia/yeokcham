...
let refusal_to_string = function
  | Invalid_identifier value -> "invalid V4 health identifier: " ^ value
  | Invalid_locator value -> "invalid V4 repair source locator: " ^ value
  | Duplicate_observation value -> "duplicate V4 health observation: " ^ value
  | No_damage -> "repair planning requires at least one diagnosed damage"
  | Invalid_expiry { created_at; expires_at } ->
      Printf.sprintf "invalid V4 repair plan expiry %Ld..%Ld" created_at
        expires_at
  | Candidate_source_mismatch ->
...
let candidate_material ~source ~object_id ~canonical_bytes_id =
  "yeokcham:v4:repair-candidate:1\000" ^ source_tag source ^ "\000"
