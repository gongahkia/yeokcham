...
module Deterministic_test_signer : Signer = struct
  let signer_identity = "yeokcham deterministic test signer"
  let algorithm = "yeokcham-test-only-not-cryptographic-v1"

  let sign release =
    "yeokcham:deterministic-test-attestation:v1\000"
...
let create_release ~parents ~workspace ~workspace_revision
  ...
  in
  let id =
    hash "yeokcham:release:v1\000" (Encoding.encode identity)
    |> release_from_digest
  in
  Ok
    {
...
let binding_domain = "yeokcham:release-binding:v1\000"
