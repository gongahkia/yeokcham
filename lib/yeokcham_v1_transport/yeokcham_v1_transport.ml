...
let publication_signature_domain = "yeokcham:v1:transport-publication:1\000"

let error_to_string = function
  | Invalid_digest value -> "invalid V1 transport digest: " ^ value
  | Invalid_remote_name value -> "invalid V1 transport remote name: " ^ value
  | Invalid_publication detail -> "invalid V1 transport publication: " ^ detail
  | Noncanonical_bytes -> "V1 transport record is not canonically encoded"
  | Duplicate_publication id -> "duplicate V1 transport publication: " ^ id
  | Missing_feed_parent id ->
      "V1 transport publication is missing feed parent: " ^ id
  | Cross_publisher_parent id ->
      "V1 transport publication parent has another publisher: " ^ id
  | Feed_cycle id -> "V1 transport publication feed has a cycle at: " ^ id
  | Repository_mismatch ->
      "V1 transport publication belongs to another repository"
  | Unknown_publisher_certificate ->
      "V1 transport publication names an unknown publisher certificate"
  | Publisher_certificate_mismatch ->
      "V1 transport publisher does not match its certificate"
  | Publisher_not_active ->
      "V1 transport publisher is not active at any current authority head"
