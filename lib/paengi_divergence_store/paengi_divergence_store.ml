...
let binding_domain = "paengi:sync-divergence-binding:v1\000"
let ( let* ) = Result.bind
...
let error_to_string = function
  ...
  | Cas_retry_exhausted attempts ->
      Printf.sprintf "sync-divergence CAS retries exhausted after %d attempts" attempts
...
let binding_fields expected = function
  ...
  | Encoding.Bool _ | Encoding.Null -> Error (Binding_error "must be an array")

let binding_integer = function
  | Encoding.Integer value -> Ok value
  | Encoding.Bytes _ | Encoding.Text _ | Encoding.Array _ | Encoding.Map _
  | Encoding.Bool _ | Encoding.Null -> Error (Binding_error "version must be an integer")

let binding_bytes label = function
  | Encoding.Bytes value -> Ok value
  | Encoding.Integer _ | Encoding.Text _ | Encoding.Array _ | Encoding.Map _
  | Encoding.Bool _ | Encoding.Null -> Error (Binding_error (label ^ " must be bytes"))
...
let encode_binding object_id =
  ...
  | Ok body -> (
      ...
      | Encoding.Array [ version; object_id ] ->
          ...
          |> Result.fold ~ok:Fun.id ~error:(fun error -> invalid_arg (error_to_string error))
      | Encoding.Integer _ | Encoding.Bytes _ | Encoding.Text _ | Encoding.Map _
...
let decode_binding bytes =
  ...
  | [ version; raw_object_id; supplied_checksum ] ->
      ...
      if not (Int64.equal version 1L) then
        Error (Binding_error (Printf.sprintf "unsupported version: %Ld" version))
      else
...
let store_set repository set =
  let* envelope =
    Divergence.envelope set |> Result.map_error (fun error -> Divergence_error error)
  in
  Store.put repository envelope |> Result.map_error (fun error -> Store_error error)

let load_set repository object_id =
  let* envelope = Store.get repository object_id |> Result.map_error (fun error -> Store_error error) in
  if Envelope.object_type envelope <> Envelope.Divergent_ref_set then
    Error
      (Unexpected_object_type
         { expected = Envelope.Divergent_ref_set; actual = Envelope.object_type envelope })
  else
...
let validate_set repository ~trusted_keys set =
  let rec collect entries = function
    ...
    | link :: rest ->
        ...
        if not (Event.Event_id.equal event_id (Divergence.link_event_id link)) then
          Error (Event_id_mismatch (Divergence.link_event_id link))
        else
          let* verified =
...
let publish repository ~trusted_keys entries =
  ...
  let components = binding_components ~ref_name:(Divergence.ref_name supplied) in
  let rec attempt remaining =
    if remaining = 0 then Error (Cas_retry_exhausted max_cas_retries)
      ...
      | None, _ | Some _, false ->
          ...
          (match
             Store.Ref_file.compare_and_swap repository ~components ~expected:current
               ~replacement
             |> Result.map_error (fun error -> Store_error error)
           with
          | Ok () -> Ok object_id
          | Error (Store_error (Store.Concurrent_ref_file_update _)) ->
              attempt (remaining - 1)
