(** Local, inspectable V1 object-store collection.
...
type root_reason =
  ...
  | Checkpoint of {
      ...
      reasons : Yeokcham_v1_model.protection_reason list;
    }
...
type error =
  ...
  | Journal_error of Yeokcham_v1_restore_journal.error
  | Proof_error of Yeokcham_v1_restore_proof.error
...
(** Reads and validates a consistent local V1 state without changing files. *)

val inspect : root:string -> (plan, error) result
(** Reads and validates the same V1 closure as [plan], but does not acquire an
