...
type error =
  ...
  | V1_store_error of V1_store.error
...
let error_to_string = function
  ...
  | V1_store_error error -> V1_store.error_to_string error
  ...
  | Invalid_snapshot_id value -> "invalid V1 snapshot object identity: " ^ value
  | Missing_reachable_object id ->
      "V1 GC reachable object is missing: " ^ Store.Stored_object_id.to_hex id
  | Duplicate_object id ->
      "V1 GC object list contains a duplicate: "
      ^ Store.Stored_object_id.to_hex id
  | Invalid_schema detail -> "invalid V1 GC transaction: " ^ detail
  | Unsupported_schema_version version ->
      Printf.sprintf "unsupported V1 GC transaction version: %Ld" version
  | Noncanonical_bytes -> "V1 GC transaction is not canonically encoded"
  | No_collectible_objects -> "V1 GC found no collectible objects"
  | Transaction_not_found id -> "V1 GC transaction not found: " ^ id
  | Transaction_collision path ->
      "V1 GC transaction path contains different bytes: " ^ path
  | Incomplete_transaction id ->
      "V1 GC transaction needs explicit resume, restore, or purge: " ^ id
  | Stale_transaction_object id ->
      "V1 GC transaction object is now reachable: "
      ^ Store.Stored_object_id.to_hex id
  | Transaction_object_missing id ->
      "V1 GC transaction object is absent from active and quarantine storage: "
      ^ Store.Stored_object_id.to_hex id
  | Quarantine_collision path ->
      "V1 GC quarantine path contains unexpected data: " ^ path
...
let is_collectible_type = function
  ...
  | Envelope.File_manifest | Envelope.V1_project_state ->
      true
...
let traverse_content store reasons content_id reachable =
  ...
  | ( Envelope.Tree | Envelope.Snapshot | Envelope.Scratch_event
    ...
    | Envelope.Peer_sync_node | Envelope.Peer_sync_conflict
    | Envelope.V1_project_state ) as object_type ->
      Error
        (Invalid_schema
...
let plan_from_loaded ~root store loaded =
  let* journal_snapshots, proof_snapshots =
    retention_snapshots ~root loaded.V1_store.project
  in
  let* roots =
    checkpoint_roots loaded.V1_store.project ~journal_snapshots ~proof_snapshots
  in
  let* reachable =
    List.fold_left
  ...
  let* objects =
    ...
    |> Result.map_error (fun error -> Store_error error)
  in
  classify ~state_head:loaded.V1_store.object_id ~objects
    ~reachable:(Object_map.bindings reachable)

let with_consistent_repository ~root action =
  let* repository =
    V1_store.open_repository ~root
    |> Result.map_error (fun error -> V1_store_error error)
  in
  ...
  Store.with_lock store ~name:"restore-retention"
    ...
    (fun () ->
      Store.with_lock store ~name:V1_store.state_head_name
        ~on_error:(fun error -> Store_error error)
        (fun () ->
          let* loaded =
            V1_store.load repository
            |> Result.map_error (fun error -> V1_store_error error)
          in
...
let transaction_directory root id =
  Filename.concat (gc_directory root) ("v1-gc-" ^ id)
  ...
  | Ok (Some _) ->
      ...
      let rec loop reversed = function
        ...
        | name :: rest when String.starts_with ~prefix:"." name ->
            loop reversed rest
        | name :: _ when not (String.starts_with ~prefix:"v1-gc-" name) ->
            Error (Invalid_schema "unknown entry in V1 GC directory")
        | name :: rest ->
