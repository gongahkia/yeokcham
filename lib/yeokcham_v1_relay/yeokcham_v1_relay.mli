(** Bounded immutable byte storage for the V1 HTTP relay.

    The relay validates identities and idempotent create semantics but has no V1
...
module V2 : sig
  (** Relay-local resumable raw-object sessions. They are deliberately outside
      V1's canonical store and history model. *)
  ...
  type error =
    ...
    | Transfer_error of Yeokcham_v1_transport.V2.error
  ...
  val start_upload :
    ...
    (Yeokcham_v1_transport.V2.transfer_session, error) result
  ...
  val receive_upload_segment :
    ...
    raw_sha256:string ->
    bytes:string ->
    (Yeokcham_v1_transport.V2.session_progress, error) result
