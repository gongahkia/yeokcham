...
type entry =
  | Directory
  | File of {
      mode : Yeokcham_snapshot.file_mode;
      content : Yeokcham_snapshot.Content.id;
    }
...
type operation =
  ...
  | Modify_content of {
      path : path;
      expected : Yeokcham_snapshot.Content.id;
      replacement : Yeokcham_snapshot.Content.id;
    }
  | Change_mode of {
      path : path;
...
module Event_id : sig
  ...
  val of_stored_object_id : Yeokcham_store.Stored_object_id.t -> t
  val stored_object_id : t -> Yeokcham_store.Stored_object_id.t
  val equal : t -> t -> bool
end
...
module Event : sig
  ...
  val create :
    ...
    resulting:Yeokcham_snapshot.Snapshot.id ->
  ...
  val base : t -> Yeokcham_snapshot.Snapshot.id
  val resulting : t -> Yeokcham_snapshot.Snapshot.id
  val operations : t -> operation list
  val source : t -> source
  val observed_at : t -> int64
  val store : Yeokcham_store.repository -> t -> (Event_id.t, error) result
  val load : Yeokcham_store.repository -> Event_id.t -> (t, error) result
end
...
module Checkpoint : sig
  ...
  val create_initial :
    snapshot:Yeokcham_snapshot.Snapshot.id -> created_at:int64 -> t
  ...
  val snapshot : t -> Yeokcham_snapshot.Snapshot.id
  val created_at : t -> int64
  val intrinsic_retention : t -> retention_reason list
  val store : Yeokcham_store.repository -> t -> (Checkpoint_id.t, error) result
  val load : Yeokcham_store.repository -> Checkpoint_id.t -> (t, error) result
end

module Cleanup_manifest : sig
  type candidate = {
    object_id : Yeokcham_store.Stored_object_id.t;
    expected_type : Yeokcham_envelope.object_type;
  }
  ...
  val store :
    Yeokcham_store.repository -> t -> (Cleanup_manifest_id.t, error) result

  val load :
    Yeokcham_store.repository -> Cleanup_manifest_id.t -> (t, error) result
end
...
module Generation : sig
  ...
  val snapshot : entry -> Yeokcham_snapshot.Snapshot.id
  ...
  val load : Yeokcham_store.repository -> Generation_id.t -> (t, error) result
...
module Retention_change : sig
  ...
  val store :
    Yeokcham_store.repository -> t -> (Retention_change_id.t, error) result

  val load :
    Yeokcham_store.repository -> Retention_change_id.t -> (t, error) result
end
...
val open_repository : Yeokcham_store.repository -> repository
...
val retain_validation_passed :
  repository ->
  Checkpoint_id.t ->
  validation:Yeokcham_id.Validation_id.t ->
  changed_at:int64 ->
  (bool, error) result
...
module Polling : sig
  ...
  val observe :
    t ->
    head:Yeokcham_snapshot.Snapshot.id ->
    observed:Yeokcham_snapshot.Snapshot.id ->
    now_ms:int64 ->
    t * bool
end
...
module Restore : sig
  ...
  val current_snapshot : plan -> Yeokcham_snapshot.Snapshot.id
