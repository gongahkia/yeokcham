...
let retention_reason_to_string = function
  ...
  | Capsule_boundary identity ->
      Printf.sprintf "capsule boundary: %s"
        (Yeokcham_id.Capsule_id.short_hex identity)
  | Release_boundary identity ->
      Printf.sprintf "release boundary: %s"
        (Yeokcham_id.Release_id.short_hex identity)
  | Validation_passed identity ->
      Printf.sprintf "validation passed: %s"
        (Yeokcham_id.Validation_id.short_hex identity)
  ...
  | Conflict_reference identity ->
      Printf.sprintf "conflict reference: %s"
        (Yeokcham_id.Conflict_id.short_hex identity)
...
let retention_value = function
  ...
  | Capsule_boundary identity ->
      value_array
        [
          Encoding.integer 1L;
          Encoding.bytes (Yeokcham_id.Capsule_id.to_bytes identity);
        ]
  | Release_boundary identity ->
      value_array
        [
          Encoding.integer 2L;
          Encoding.bytes (Yeokcham_id.Release_id.to_bytes identity);
        ]
  | Validation_passed identity ->
      value_array
        [
          Encoding.integer 3L;
          Encoding.bytes (Yeokcham_id.Validation_id.to_bytes identity);
        ]
  ...
  | Conflict_reference identity ->
      value_array
        [
          Encoding.integer 6L;
          Encoding.bytes (Yeokcham_id.Conflict_id.to_bytes identity);
        ]
...
let ensure_identity raw constructor =
  if String.length raw <> 32 then
    ...
    | Error error ->
        Error (Invalid_schema (Yeokcham_id.parse_error_to_string error))
...
let retention_of_value value =
  ...
  | [ tag; raw ] -> (
      ...
      match tag with
      | 1L ->
          ensure_identity raw Yeokcham_id.Capsule_id.of_bytes
          |> Result.map (fun value -> Capsule_boundary value)
      | 2L ->
          ensure_identity raw Yeokcham_id.Release_id.of_bytes
          |> Result.map (fun value -> Release_boundary value)
      | 3L ->
          ensure_identity raw Yeokcham_id.Validation_id.of_bytes
          |> Result.map (fun value -> Validation_passed value)
      | 6L ->
          ensure_identity raw Yeokcham_id.Conflict_id.of_bytes
        ...
        (List.exists
           (function
             | Capsule_boundary existing ->
                 Yeokcham_id.Capsule_id.equal existing capsule
...
module Restore = struct
  ...
  let temporary_path directory basename attempt =
    Filename.concat directory
      (Printf.sprintf ".%s.yeokcham-restore-%d-%d" basename (Unix.getpid ())
         attempt)
