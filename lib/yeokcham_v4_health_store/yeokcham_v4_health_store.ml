...
let error_to_string = function
  ...
  | Invalid_plan_id id -> "invalid V4 repair plan identifier: " ^ id
  | Metadata_missing path -> "V4 metadata directory is missing: " ^ path
  | Invalid_metadata_path path -> "invalid V4 repair metadata path: " ^ path
  | Plan_collision path ->
      "V4 repair plan path contains different bytes: " ^ path
  | Not_found id -> "V4 repair plan is absent: " ^ id
  | Io_error { operation; path; message } ->
      Printf.sprintf "V4 repair plan %s %s: %s" operation path message

let directory ~root =
  Filename.concat (Filename.concat root ".yeokcham") "repair-plans"

let file_name id = "v4-repair-plan-" ^ id ^ ".cbor"
