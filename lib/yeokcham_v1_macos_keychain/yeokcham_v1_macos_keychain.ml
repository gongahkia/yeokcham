...
external lookup_raw : string -> string -> int * string option
  = "caml_yeokcham_v1_macos_keychain_lookup"

external store_raw : string -> string -> string -> int
  = "caml_yeokcham_v1_macos_keychain_store"

let service_name = "io.github.gongahkia.yeokcham.v1.device-signing"
let account_prefix = "v1-ed25519-device-1/"
...
let error_to_string = function
  ...
  | Keychain_item_missing ->
      "local V1 signing key is missing from macOS Keychain"
  | Keychain_item_conflict ->
      "macOS Keychain already contains different material for this V1 device"
  | Invalid_keychain_material ->
      "macOS Keychain returned invalid V1 signing material"
