...
let error_to_string = function
  ...
  | Invalid_schema detail -> "invalid V1 state schema: " ^ detail
  | Unsupported_schema_version version ->
      Printf.sprintf "unsupported V1 state schema version: %Ld" version
  | Model_error error -> Model.error_to_string error
  | Noncanonical_bytes -> "V1 state record is not canonically encoded"
...
let decode_state_value value =
  let* fields = array_values "V1 state" value in
  match fields with
  | [
   version;
   creator;
   ...
   usernames;
      ...
      let* version = decoded_integer "V1 state schema version" version in
  ...
  | _ -> Error (Invalid_schema "V1 state has an unsupported field count")
