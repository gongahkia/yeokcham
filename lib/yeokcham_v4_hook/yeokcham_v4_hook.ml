...
let error_to_string = function
  | Invalid_id value -> "invalid V4 hook ID: " ^ value
  | Invalid_argv value -> "invalid V4 hook argv: " ^ value
  | Duplicate_hook value -> "V4 hook already exists: " ^ value
  | Unknown_hook value -> "V4 hook does not exist: " ^ value
  | Unsupported_version value ->
      Printf.sprintf "unsupported V4 hooks version: %Ld" value
  | Invalid_encoding value -> "invalid V4 hooks encoding: " ^ value
  | Noncanonical_encoding -> "V4 hooks bytes are noncanonical"
...
let id_of ~event ~argv =
  Hash.feed_string Hash.empty "yeokcham:v4:hook:v1\000" |> fun context ->
