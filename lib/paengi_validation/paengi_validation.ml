...
let raw_snapshot identity =
  Snapshot.Snapshot.stored_object_id identity |> Store.Stored_object_id.to_raw_bytes
...
let is_sorted_unique_environment environment =
  let rec loop previous = function
    ...
    | (name, _) :: rest -> (
        ...
        | Some previous -> String.compare previous name < 0 && loop (Some name) rest)
  in
...
let make_command command =
  ...
  else if String.is_empty command.executable || String.contains command.executable '\000' then
    Error (Invalid_command "executable must be nonempty and contain no NUL")
  ...
  else if List.exists (fun value -> not (valid_component value)) command.working_directory then
    Error (Invalid_command "working directory must use safe relative components")
  else if
    List.exists
      (fun (name, value) ->
...
let environment_policy_of_code = function
  | 0L -> Ok Empty
  | 1L -> Ok Inherit
  | value -> Error (Decode_error (Printf.sprintf "unknown environment policy: %Ld" value))
...
let fields name expected = function
  ...
  | Encoding.Array _ -> Error (Decode_error (Printf.sprintf "%s has wrong arity" name))
...
let parse_nonnegative_int name value =
  ...
  if Int64.compare value 0L < 0 || Int64.compare value (Int64.of_int max_int) > 0 then
    Error (Decode_error (name ^ " is out of range"))
...
let decode_command_payload value =
  ...
  match values with
  | [ version; executable; arguments; working_directory; timeout_ms; max_stdout_bytes;
      max_stderr_bytes; environment_policy; environment; retain_output;
      mandatory_features ] ->
      ...
      let* arguments =
        ...
        |> Result.map List.rev
      in
      let* working_directory = parse_array "validation working directory" working_directory in
      ...
      let* max_stdout_bytes = parse_nonnegative_int "validation stdout limit" max_stdout_bytes in
      let* max_stderr_bytes = parse_nonnegative_int "validation stderr limit" max_stderr_bytes in
      let* environment_policy = integer "validation environment policy" environment_policy in
      ...
      let* mandatory_features = integer "validation command mandatory features" mandatory_features in
      let command =
        {
          executable;
          arguments;
...
let status_of_code = function
  ...
  | value -> Error (Decode_error (Printf.sprintf "unknown validation status: %Ld" value))

let optional_integer = function
  | None -> Encoding.null
  | Some value -> Encoding.integer (Int64.of_int value)

let optional_raw_bytes = function None -> Encoding.null | Some value -> Encoding.bytes value
...
let evidence_identity_value ~snapshot ~command ~command_index
  ...
  let environment_fingerprint = optional_raw_bytes result.runner_environment_fingerprint in
  value_array
    [
      Encoding.integer 1L;
...
let validate_stream name limit stream =
  if String.length stream.digest <> Hash.digest_size then
    ...
    && not (String.equal stream.digest (Hash.digest_string stream.retained |> Hash.to_raw_string))
  then Error (Invalid_evidence (name ^ " digest disagrees with complete retained output"))
  else Ok ()

let validate_result command (result : process_result) =
  let* () = validate_stream "stdout" command.max_stdout_bytes result.runner_stdout in
  let* () = validate_stream "stderr" command.max_stderr_bytes result.runner_stderr in
  if Int64.compare result.runner_duration_ms 0L < 0 then
    ...
    | Passed when result.runner_exit_code = Some 0 && result.runner_signal = None && result.runner_execution_error = None -> Ok ()
    | Failed when result.runner_execution_error = None -> Ok ()
    | Timed_out when result.runner_exit_code = None && result.runner_execution_error = None -> Ok ()
    | Execution_error when result.runner_exit_code = None && result.runner_signal = None && Option.is_some result.runner_execution_error -> Ok ()
    | Passed -> Error (Invalid_evidence "passed evidence must record exit code zero")
    | Failed -> Error (Invalid_evidence "failed evidence must not contain an execution error")
    | Timed_out -> Error (Invalid_evidence "timeout evidence has incompatible status fields")
    | Execution_error -> Error (Invalid_evidence "execution error requires an error message")

let create_evidence ~snapshot ~command ~command_index ~result ~stdout_output
    ~stderr_output ~observed_at =
  let* command = make_command command in
  if command_index < 0 then Error (Invalid_evidence "command index must be non-negative")
  else
    ...
    in
    let id =
      hash "paengi:validation-evidence:v1\000" (Encoding.encode identity)
      |> id_from_digest
    in
    Ok
      {
...
let evidence_environment_fingerprint (evidence : evidence) = evidence.environment_fingerprint
let evidence_runner_format_version (evidence : evidence) = evidence.runner_format_version
...
let evidence_payload evidence =
  ...
  in
  let* identity =
    evidence_identity_value ~snapshot:evidence.snapshot ~command:evidence.command
      ~command_index:evidence.command_index ~result
      ~stdout_output:evidence.stdout_output ~stderr_output:evidence.stderr_output
  in
  let derived =
    hash "paengi:validation-evidence:v1\000" (Encoding.encode identity)
    |> id_from_digest
  in
  if not (Id.Validation_id.equal evidence.id derived) then Error Logical_identity_mismatch
  else
    let* command = command_payload evidence.command in
    let* execution_error = optional_text evidence.execution_error in
    let environment_fingerprint = optional_raw_bytes evidence.environment_fingerprint in
    value_array
      [
        Encoding.integer 1L;
...
let decode_evidence_payload value =
  ...
  match values with
  | [ version; supplied_id; snapshot; command; command_index; status; exit_code;
      signal; execution_error; duration_ms; stdout_digest; stderr_digest;
      stdout_truncated; stderr_truncated; stdout_output; stderr_output;
      environment_fingerprint; runner_format_version; observed_at ] ->
      let* version = integer "validation evidence version" version in
      if version <> 1L then Error (Unsupported_schema_version version)
      else
        let* supplied_id = parse_validation "validation evidence ID" supplied_id in
        let* snapshot = parse_snapshot "validation snapshot ID" snapshot in
        let* command = decode_command_payload command in
        let* command_index = parse_nonnegative_int "validation command index" command_index in
        let* status =
          integer "validation status" status |> fun result ->
          Result.bind result status_of_code
        in
        let* exit_code = parse_optional_integer "validation exit code" exit_code in
        let* signal = parse_optional_integer "validation signal" signal in
        let* execution_error = parse_optional_text "validation execution error" execution_error in
        ...
        let* stdout_truncated = boolean "validation stdout truncation" stdout_truncated in
        let* stderr_truncated = boolean "validation stderr truncation" stderr_truncated in
        let* stdout_output = parse_optional_stored "validation stdout output" stdout_output in
        let* stderr_output = parse_optional_stored "validation stderr output" stderr_output in
        let* environment_fingerprint = parse_optional_raw_bytes "validation environment fingerprint" environment_fingerprint in
        let* runner_format_version = integer "validation runner format version" runner_format_version in
        ...
        if runner_format_version <> 1L then
          ...
          let result : process_result =
            {
              ...
              runner_stdout =
                { digest = stdout_digest; retained = ""; truncated = stdout_truncated };
              runner_stderr =
                { digest = stderr_digest; retained = ""; truncated = stderr_truncated };
              runner_environment_fingerprint = environment_fingerprint;
            }
          in
          let* () =
            if String.length stdout_digest <> Hash.digest_size
               || String.length stderr_digest <> Hash.digest_size
            then Error (Invalid_evidence "stream digest must be 32 bytes")
            else
              match status with
              | Passed when exit_code = Some 0 && signal = None && execution_error = None -> Ok ()
              | Failed when execution_error = None -> Ok ()
              | Timed_out when exit_code = None && execution_error = None -> Ok ()
              | Execution_error when exit_code = None && signal = None && Option.is_some execution_error -> Ok ()
          ...
          let derived =
            ...
            |> id_from_digest
          in
          if not (Id.Validation_id.equal supplied_id derived) then Error Logical_identity_mismatch
          else
            let evidence =
              {
                id = supplied_id;
            ...
            else Error (Decode_error "validation evidence bytes are noncanonical")
  | _ -> assert false
...
let validate_output store = function
  | None -> Ok ()
  | Some object_id ->
      Snapshot.Content.load store (Snapshot.Content.of_stored_object_id object_id)
  ...
  if Envelope.object_type envelope <> Envelope.Validation then
    Error
      (Unexpected_object_type
         { expected = Envelope.Validation; actual = Envelope.object_type envelope })
  else
...
let fresh_directory () =
  try
    let path = Filename.temp_file "paengi-validation-" "" in
    Sys.remove path;
...
let fingerprint_environment environment =
  ...
  |> hash "paengi:validation-environment:v1\000" |> Option.some
...
module Unix_runner : Process_runner = struct
  ...
  let feed accumulator bytes length =
    accumulator.context <- Hash.feed_bytes accumulator.context ~off:0 ~len:length bytes;
    let available = accumulator.limit - Buffer.length accumulator.retained_bytes in
    if available > 0 then Buffer.add_subbytes accumulator.retained_bytes bytes 0 (min available length);
  ...
  let environment command =
    let inherited = match command.environment_policy with Empty -> [] | Inherit -> Array.to_list (Unix.environment ()) in
    ...
    |> List.rev_append without_overrides |> Array.of_list
    ...
    if child = 0 then (
      ...
      (try
       ...
       with Unix.Unix_error (error, function_name, argument) ->
         let message = Unix.error_message error ^ ": " ^ function_name ^ " " ^ argument in
         ignore (Unix.write_substring error_write message 0 (String.length message));
      ...
      let read_stream descriptor accumulator open_ =
        ...
        let rec loop () =
          try
            ...
            | 0 -> Unix.close descriptor; open_ := false
            | length -> feed accumulator buffer length; loop ()
          with
          | Unix.Unix_error ((Unix.EAGAIN | Unix.EWOULDBLOCK), _, _) -> ()
          | Unix.Unix_error _ -> Unix.close descriptor; open_ := false
        in
        loop ()
      in
      let read_error () =
        ...
        let rec loop () =
          try
            ...
            | 0 -> Unix.close error_read; open_error := false
            | length -> Buffer.add_subbytes error_text buffer 0 length; loop ()
          with
          | Unix.Unix_error ((Unix.EAGAIN | Unix.EWOULDBLOCK), _, _) -> ()
          | Unix.Unix_error _ -> Unix.close error_read; open_error := false
        in
        loop ()
      in
      while Option.is_none !process_status || !open_stdout || !open_stderr || !open_error do
        ...
        if
          ...
          && not !sent_kill
          && Option.is_none !process_status
          && Option.fold ~none:false ~some:(fun at -> now -. at >= 0.1) !terminated_at
        then (
          sent_kill := true;
        ...
        if descriptors <> [] then (
          ...
          if List.mem stdout_read ready then read_stream stdout_read stdout open_stdout;
          if List.mem stderr_read ready then read_stream stderr_read stderr open_stderr;
          if List.mem error_read ready then read_error ())
        else if Option.is_none !process_status then ignore (Unix.select [] [] [] 0.01)
      done;
      let duration_ms = Int64.of_float ((Unix.gettimeofday () -. started) *. 1000.) in
      let status, exit_code, signal, execution_error =
        ...
        if not (String.is_empty execution_error) then
          ...
          | None ->
              (Execution_error, None, None, Some "process status was unavailable")
      in
      {
        runner_status = status;
        runner_exit_code = exit_code;
        ...
        runner_environment_fingerprint = fingerprint_environment (environment command);
      })
end
...
let retain_output store command stream =
  if not command.retain_output then Ok None
    ...
    |> Result.map (fun output -> Some (Snapshot.Content.stored_object_id output))
    |> Result.map_error (fun error -> Snapshot_error error)
...
let run ?(runner = (module Unix_runner : Process_runner)) ~store ~snapshot
  ...
  let finished =
    try
      match Snapshot.Materialize.write ~destination:temporary store materialised with
      ...
      | Ok () ->
          ...
          in
          let* stdout_output = retain_output store command result.runner_stdout in
          let* stderr_output = retain_output store command result.runner_stderr in
          let* evidence =
