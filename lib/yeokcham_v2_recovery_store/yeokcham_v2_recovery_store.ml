...
let filename = "recovery-package-v1.cbor"
let metadata_name = ".yeokcham"
let directory_name = "recovery"
...
let error_to_string = function
  ...
  | Not_v2_root classification ->
      "V2 recovery persistence requires a V2-only root, found "
      ^ Cutover.classification_to_string classification
  | Recovery_error error -> Recovery.error_to_string error
  | Missing_recovery_package path -> "recovery package is missing: " ^ path
  | Invalid_recovery_path path -> "invalid recovery package path: " ^ path
  | Unexpected_recovery_entry path ->
      "unexpected recovery directory entry: " ^ path
  | Recovery_already_initialized path ->
      "recovery package already contains different canonical bytes: " ^ path
  | Io_error { operation; path; message } ->
      Printf.sprintf "%s failed for %s: %s" operation path message
  | Temporary_name_exhausted directory ->
      "recovery package staging namespace is exhausted: " ^ directory

let recovery_directory ~root =
  Filename.concat (Filename.concat root metadata_name) directory_name

let recovery_path ~root = Filename.concat (recovery_directory ~root) filename
...
let is_temporary_filename name =
  let prefix = "." ^ filename ^ ".recovery-" in
...
let rec ensure_recovery_directory ~root =
  let directory = recovery_directory ~root in
  match lstat directory with
  | Ok (Some stat) when stat.Unix.st_kind = Unix.S_DIR -> Ok directory
  | Ok (Some _) -> Error (Invalid_recovery_path directory)
  ...
  | Ok None -> (
      try
        ...
        Ok directory
      with
      | Unix.Unix_error (Unix.EEXIST, _, _) -> ensure_recovery_directory ~root
      | Unix.Unix_error (error, _, _) ->
          Error (io_error ~operation:"mkdir" ~path:directory error))

let recovery_directory_entries ~root =
  let directory = recovery_directory ~root in
  let* stat =
    match lstat directory with
    | Ok (Some stat) -> Ok stat
    | Ok None -> Error (Missing_recovery_package (recovery_path ~root))
    | Error error -> Error error
  in
  if stat.Unix.st_kind <> Unix.S_DIR then
    Error (Invalid_recovery_path directory)
  else
    let* names =
    ...
    let rec validate = function
      ...
      | name :: rest ->
          ...
          if not (String.equal name filename || is_temporary_filename name) then
            Error (Unexpected_recovery_entry path)
          else
            let* stat =
              match lstat path with
              | Ok (Some stat) -> Ok stat
              | Ok None -> Error (Invalid_recovery_path path)
              | Error error -> Error error
            in
            if stat.Unix.st_kind <> Unix.S_REG then
              Error (Invalid_recovery_path path)
            else validate rest
    in
...
let read_regular_file path =
  let* stat =
    match lstat path with
    | Ok (Some stat) -> Ok stat
    | Ok None -> Error (Missing_recovery_package path)
    | Error error -> Error error
  in
  if
    stat.Unix.st_kind <> Unix.S_REG
    || stat.Unix.st_size > Recovery.max_package_bytes
  then Error (Invalid_recovery_path path)
  else
    try
      let descriptor = Unix.openfile path [ Unix.O_RDONLY ] 0 in
      Fun.protect
        ...
        (fun () ->
          ...
          let rec read offset =
            if offset = stat.Unix.st_size then Ok ()
              ...
              try
                ...
                in
                if count = 0 then Error (Invalid_recovery_path path)
          ...
          else Error (Invalid_recovery_path path))
    with Unix.Unix_error (error, _, _) ->
...
let temporary_path directory attempt =
  Filename.concat directory
    (Printf.sprintf ".%s.recovery-%d-%d" filename (Unix.getpid ()) attempt)
...
let read_package ~root =
  let* () = check_v2_root root in
  let* _ = recovery_directory_entries ~root in
  let* bytes = read_regular_file (recovery_path ~root) in
  Recovery.decode bytes |> Result.map_error (fun error -> Recovery_error error)

let initialize ~root package =
  let* () = check_v2_root root in
  let* directory = ensure_recovery_directory ~root in
  let* () = recovery_directory_entries ~root |> Result.map (fun _ -> ()) in
  let final = recovery_path ~root in
  let expected = Recovery.encode package in
  match lstat final with
  | Error error -> Error error
  | Ok (Some _) ->
      let* actual = read_regular_file final in
      if String.equal actual expected then Ok Already_initialized
      else Error (Recovery_already_initialized final)
  | Ok None -> (
