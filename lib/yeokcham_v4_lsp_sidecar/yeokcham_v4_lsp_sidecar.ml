...
let snapshot_role_to_string = function Base -> "base" | Left -> "left" | Right -> "right"
...
let with_temporary_directory run =
  ...
  Fun.protect ~finally:(fun () -> remove_tree directory) (fun () -> run directory)
...
type output = { output_descriptor : Unix.file_descr; mutable output_deadline : float }
...
let read_line (input : input) =
  ...
  let rec loop () =
    ...
    | None -> if Buffer.length buffer = 0 then None else raise (Session_error "truncated LSP header")
...
let read_exactly (input : input) length =
  ...
  let rec loop offset =
    if offset = length then Some (Bytes.unsafe_to_string bytes)
      ...
      wait_readable input;
      try
        let read = Unix.read input.input_descriptor bytes offset (length - offset) in
        if read = 0 then None
        else (
          ...
          if input.received_bytes > maximum_total_bytes then
            raise (Session_error "LSP output exceeded 2 MiB");
          loop (offset + read))
      with Unix.Unix_error ((Unix.EAGAIN | Unix.EWOULDBLOCK), _, _) -> loop offset)
  in
  loop 0
...
let write_all (output : output) value =
  let rec loop offset =
    if offset = String.length value then ()
      ...
      | _ ->
          try
            let written = Unix.write_substring output.output_descriptor value offset (String.length value - offset) in
            if written = 0 then raise (Session_error "LSP stdin write returned zero");
            loop (offset + written)
          with Unix.Unix_error ((Unix.EAGAIN | Unix.EWOULDBLOCK), _, _) -> loop offset
  in
  loop 0

module Rpc_io = Lsp.Io.Make (Sync) (struct
...
let range_of_lsp (range : Range.t) : range =
  {
    start = position_of_lsp range.start;
    end_ = position_of_lsp range.end_;
  }
...
let range_nearby left right =
  ...
  in
  before_or_equal left.start right.end_ && before_or_equal right.start left.end_
  || (left.start.line = right.end_.line && abs (left.start.character - right.end_.character) <= 1)
  || (right.start.line = left.end_.line && abs (right.start.character - left.end_.character) <= 1)
...
let relative_path ~workspace uri =
  ...
  if String.starts_with ~prefix candidate then
    Some (String.sub candidate (String.length prefix) (String.length candidate - String.length prefix))
  else None
...
let locations_of_lsp ~workspace locations =
  ...
  |> fun locations -> if List.length locations > maximum_locations then raise (Session_error "LSP returned too many locations") else locations
...
let locations_of_definition ~workspace = function
  ...
  | `LocationLink links ->
      ...
      |> List.filter_map (fun link ->
             match relative_path ~workspace link.Lsp.Types.LocationLink.targetUri with
             | None -> None
...
let server_capabilities result =
  ...
  let enabled = function Some (`Bool false) | None -> false | Some _ -> true in
  let document_symbol = enabled capabilities.Lsp.Types.ServerCapabilities.documentSymbolProvider in
  let definition = enabled capabilities.Lsp.Types.ServerCapabilities.definitionProvider in
  let references = enabled capabilities.Lsp.Types.ServerCapabilities.referencesProvider in
  let workspace_symbol = enabled capabilities.Lsp.Types.ServerCapabilities.workspaceSymbolProvider in
  let names =
    [ ("document-symbol", document_symbol); ("definition", definition); ("references", references); ("workspace-symbol", workspace_symbol) ]
    |> List.filter_map (fun (name, present) -> if present then Some name else None)
  in
  if List.length names <> 4 then
    raise (Session_error "configured server lacks a required read-only LSP capability");
  names

let client_capabilities () =
  let document_symbol = Lsp.Types.DocumentSymbolClientCapabilities.create ~hierarchicalDocumentSymbolSupport:true () in
  let text_document =
  ...
  in
  let workspace = Lsp.Types.WorkspaceClientCapabilities.create ~applyEdit:false ~configuration:false ~workspaceFolders:false () in
  let general = Lsp.Types.GeneralClientCapabilities.create ~positionEncodings:[ Lsp.Types.PositionEncodingKind.UTF16 ] () in
  Lsp.Types.ClientCapabilities.create ~general ~textDocument:text_document ~workspace ()
  ...
  try
    ...
    | 0 ->
        (try
           ...
           List.iter close_noerr [ stdin_read; stdin_write; stdout_read; stdout_write; null ];
           Unix.execv server.program (Array.of_list (server.program :: server.arguments))
         with _ -> exit 127)
    | process ->
...
let reject_server_request output request =
  let error = Jsonrpc.Response.Error.make ~code:Jsonrpc.Response.Error.Code.MethodNotFound ~message:"Yeokcham semantic sidecars refuse server-initiated state changes" () in
  send_packet output (Jsonrpc.Packet.Response (Jsonrpc.Response.error request.Jsonrpc.Request.id error))
...
let rec wait_for_response input output identifier =
  ...
  | Some (Jsonrpc.Packet.Batch_response responses) ->
      (match List.find_opt (fun response -> Jsonrpc.Id.equal response.Jsonrpc.Response.id identifier) responses with
  ...
  | Some (Jsonrpc.Packet.Batch_call calls) ->
      List.iter
        ...
        calls;
      wait_for_response input output identifier
  | Some (Jsonrpc.Packet.Notification _) -> wait_for_response input output identifier

let request input output counter request =
  if !counter >= maximum_positions then raise (Session_error "LSP request limit exceeded");
  incr counter;
  ...
  | Ok result ->
      (try Lsp.Client_request.response_of_json request result
       with Jsonrpc.Json.Of_json _ -> raise (Session_error "LSP returned malformed result"))
  | Error error -> raise (Session_error ("LSP request failed: " ^ error.Jsonrpc.Response.Error.message))

let notify_initialized output =
  send_packet output (Jsonrpc.Packet.Notification (Lsp.Client_notification.to_jsonrpc Lsp.Client_notification.Initialized))

let flatten_document_symbols symbols =
  let rec loop ancestry reversed = function
    | [] -> List.rev reversed
    | symbol :: rest ->
        let current = (symbol.Lsp.Types.DocumentSymbol.name, symbol.Lsp.Types.DocumentSymbol.kind, ancestry, symbol.Lsp.Types.DocumentSymbol.range, symbol.Lsp.Types.DocumentSymbol.selectionRange) in
        let nested = match symbol.Lsp.Types.DocumentSymbol.children with None -> [] | Some children -> loop (ancestry @ [ symbol.name ]) [] children in
...
let document_symbol_entries ~workspace response =
  ...
  | Some (`SymbolInformation symbols) ->
      ...
      |> List.filter_map (fun symbol ->
             match relative_path ~workspace symbol.Lsp.Types.SymbolInformation.location.uri with
             | None -> None
             | Some _ ->
                 Some
                   ( symbol.name,
                     symbol.kind,
                     (match symbol.containerName with None -> [] | Some value -> [ value ]),
                     symbol.location.range,
                     symbol.location.range ))

let safe_regular_file path =
  try (Unix.lstat path).Unix.st_kind = Unix.S_REG with Unix.Unix_error _ -> false

let files_for_workspace ~workspace paths =
  paths
  |> List.filter (fun path -> safe_regular_file (Filename.concat workspace path))
  |> List.sort_uniq String.compare
  |> fun files -> if List.length files > maximum_documents then raise (Session_error "semantic document limit exceeded") else files
...
let utf8_width bytes offset limit =
  ...
  if offset >= limit then None
    ...
    | value when value land 0xe0 = 0xc0 && offset + 1 < limit && byte (offset + 1) land 0xc0 = 0x80 ->
        Some (2, 1)
    | value when value land 0xf0 = 0xe0 && offset + 2 < limit && byte (offset + 1) land 0xc0 = 0x80 && byte (offset + 2) land 0xc0 = 0x80 ->
        Some (3, 1)
    | value when value land 0xf8 = 0xf0 && offset + 3 < limit && byte (offset + 1) land 0xc0 = 0x80 && byte (offset + 2) land 0xc0 = 0x80 && byte (offset + 3) land 0xc0 = 0x80 ->
        Some (4, 2)
    | _ -> None
  ...
  | Some start ->
      ...
      let rec character offset units =
        if units = position.character then Some offset
          ...
          | None -> None
          | Some (width, code_units) -> character (offset + width) (units + code_units)
      in
      character start 0

let exact_slice bytes range =
  match (byte_offset_of_position bytes range.start, byte_offset_of_position bytes range.end_) with
  | Some start, Some end_ when start <= end_ -> Some (String.sub bytes start (end_ - start))
  | _ -> None

let inspect_workspace ~store server ~role ~snapshot_id ~snapshot ~paths =
  with_temporary_directory (fun workspace ->
      match Snapshot.Materialize.write ~destination:workspace store snapshot with
      | Error error -> raise (Session_error (Snapshot.Materialize.error_to_string error))
      | Ok () ->
          let process, stdin, stdout = start_process ~workspace server in
          let input = { input_descriptor = stdout; input_deadline = Unix.gettimeofday () +. initialization_timeout; received_bytes = 0 } in
          let output = { output_descriptor = stdin; output_deadline = input.input_deadline } in
          Fun.protect
            ~finally:(fun () ->
              close_noerr stdin;
              close_noerr stdout;
            ...
            (fun () ->
              ...
              let initialized =
                Lsp.Types.InitializeParams.create
                  ...
                  ~clientInfo:(Lsp.Types.InitializeParams.create_clientInfo ~name:"yeokcham" ~version:"v4" ())
                  ~processId:(Unix.getpid ()) ~rootUri:root_uri ()
              in
              let result = request input output counter (Lsp.Client_request.Initialize initialized) in
                  ...
                  let entries =
                    request input output counter
                      (Lsp.Client_request.DocumentSymbol
                         (Lsp.Types.DocumentSymbolParams.create ~textDocument:document ()))
                    ...
                    (fun (name, kind, ancestry, range, selection_range) ->
                      ...
                      let definitions =
                        request input output counter
                          (Lsp.Client_request.TextDocumentDefinition
                             (Lsp.Types.DefinitionParams.create ~position ~textDocument:document ()))
                        ...
                        request input output counter
                          (Lsp.Client_request.TextDocumentReferences
                             (Lsp.Types.ReferenceParams.create
                                ~context:(Lsp.Types.ReferenceContext.create ~includeDeclaration:true)
                                ~position ~textDocument:document ()))
                        |> Option.value ~default:[] |> locations_of_lsp ~workspace
                      in
                      let workspace_matches =
                        ...
                        |> List.filter_map (fun match_ ->
                               match relative_path ~workspace match_.Lsp.Types.SymbolInformation.location.uri with
                               | None -> None
                               | Some candidate -> Some (candidate, range_of_lsp match_.location.range))
                      in
                      symbols :=
                        {
                          observation =
              ...
              ( List.rev !symbols,
                {
                  ...
                  reported_name = Option.map (fun info -> info.Lsp.Types.InitializeResult.name) server_info;
                  reported_version = Option.bind server_info (fun info -> info.version);
                  capabilities;
                } )))
...
let changed_same_symbol ~base (left : measured_symbol) (right : measured_symbol) =
  ...
  && match List.find_opt (fun candidate -> symbol_key candidate.observation = key) base with
     | None -> false
     | Some baseline ->
         (match
         ...
         | Some baseline, Some left, Some right ->
             not (String.equal baseline left) && not (String.equal baseline right)
         | None, _, _ | _, None, _ | _, _, None -> false)

let shared_locations (left : symbol) (right : symbol) =
  List.exists (fun location -> List.mem location right.definitions || List.mem location right.references) left.definitions
  || List.exists (fun location -> List.mem location right.definitions || List.mem location right.references) left.references
  ...
  let pairs =
    ...
    |> List.concat_map (fun left_symbol ->
           ...
           |> List.filter_map (fun right_symbol ->
                  if symbol_key left_symbol.observation = symbol_key right_symbol.observation then Some (left_symbol, right_symbol) else None))
  in
  pairs
  |> List.filter_map (fun (left_symbol, right_symbol) ->
         let evidence =
           if changed_same_symbol ~base left_symbol right_symbol then Some Same_symbol_changed
           else
             match sensitivity with
             | Config.Same_symbol -> None
             | Config.Nearby_ranges when left_symbol.observation.path = right_symbol.observation.path && range_nearby left_symbol.observation.range right_symbol.observation.range -> Some Nearby_returned_ranges
             | Config.References when shared_locations left_symbol.observation right_symbol.observation -> Some Shared_definition_or_reference
           ...
           (fun evidence ->
             {
               ...
               symbol = String.concat "." (left_symbol.observation.ancestry @ [ left_symbol.observation.name ]);
               evidence;
...
let inspect ~store ~server ~base ~left ~right ~paths =
  let paths = paths |> List.filter (Config.matches_path server) |> List.sort_uniq String.compare in
  if paths = [] then Unavailable { server = server.name; reason = "no changed paths match this server" }
  else
    try
      let base_id, base_snapshot = base in
      let left_id, left_snapshot = left in
      let right_id, right_snapshot = right in
      let base_symbols, base_details = inspect_workspace ~store server ~role:Base ~snapshot_id:base_id ~snapshot:base_snapshot ~paths in
      let left_symbols, _ = inspect_workspace ~store server ~role:Left ~snapshot_id:left_id ~snapshot:left_snapshot ~paths in
      let right_symbols, _ = inspect_workspace ~store server ~role:Right ~snapshot_id:right_id ~snapshot:right_snapshot ~paths in
      let possible_overlaps = overlaps server.overlap_sensitivity ~base:base_symbols ~left:left_symbols ~right:right_symbols in
      Available
        {
          server = base_details;
          snapshots = [ (Base, base_id); (Left, left_id); (Right, right_id) ];
          symbols = List.map (fun measured -> measured.observation) (base_symbols @ left_symbols @ right_symbols);
          possible_overlaps;
        }
    with
    | Session_error reason -> Unavailable { server = server.name; reason }
    | Unix.Unix_error (error, operation, path) ->
        Unavailable { server = server.name; reason = Printf.sprintf "%s %s: %s" operation path (Unix.error_message error) }
    | Lsp.Io.Error reason -> Unavailable { server = server.name; reason }
    | Jsonrpc.Json.Of_json _ -> Unavailable { server = server.name; reason = "malformed JSON-RPC packet" }
    | Yojson.Json_error _ -> Unavailable { server = server.name; reason = "malformed JSON-RPC packet" }
