...
let error_to_string = function
  | Invalid_operation_id value -> "invalid V1 restore operation id: " ^ value
  | Identical_snapshots ->
      "V1 in-place restore safety and target snapshots are identical"
  | Invalid_generation generation ->
      Printf.sprintf "invalid V1 restore journal generation: %Ld" generation
  | Invalid_transition { previous; next } ->
      Printf.sprintf "invalid V1 restore transition: %s to %s"
  ...
  | Invalid_schema detail -> "invalid V1 restore journal: " ^ detail
  | Unsupported_schema_version version ->
      Printf.sprintf "unsupported V1 restore journal version: %Ld" version
  | Noncanonical_bytes -> "V1 restore journal is not canonically encoded"
  | Journal_collision path ->
      "V1 restore journal path contains different bytes: " ^ path
...
let filename journal =
  Printf.sprintf "v1-restore-%s-%Ld.cbor" journal.operation_id
    journal.generation
...
let is_record_name name =
  String.starts_with ~prefix:"v1-restore-" name
