...
let error_to_string = function
  ...
  | Identical_snapshots ->
      "V1 restore proof safety and target snapshots are identical"
  | Encoding_error error -> Encoding.construction_error_to_string error
  | Decode_error error -> Encoding.decode_error_to_string error
  | Invalid_schema detail -> "invalid V1 restore proof: " ^ detail
  | Unsupported_schema_version version ->
      Printf.sprintf "unsupported V1 restore proof version: %Ld" version
  | Noncanonical_bytes -> "V1 restore proof is not canonically encoded"
  | Proof_collision path ->
      "V1 restore proof path contains different bytes: " ^ path
  | Not_found operation -> "V1 restore proof not found: " ^ operation
...
let filename proof =
  Printf.sprintf "v1-restore-proof-%s.cbor" proof.operation_id
...
let path_for_operation root operation_id =
  Filename.concat (proof_directory root)
    (Printf.sprintf "v1-restore-proof-%s.cbor" operation_id)
...
let is_record_name name =
  String.starts_with ~prefix:"v1-restore-proof-" name
