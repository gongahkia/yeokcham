...
let secret_prefix = "v1ra1_"
let registry_name = ".relay-access-v1.cbor"
let lock_name = ".relay-access-v1.lock"
let ( let* ) = Result.bind

let error_to_string = function
  | Invalid_repository -> "invalid V1 relay access repository ID"
  | Invalid_credential_id -> "invalid V1 relay access credential ID"
  | Invalid_scope -> "invalid V1 relay access scope"
  | Invalid_lifetime -> "invalid V1 relay access lifetime"
  | Unknown_credential -> "unknown V1 relay access credential"
  | Credential_inactive -> "V1 relay access credential is inactive"
  | Entropy_failure -> "V1 relay access OS CSPRNG is unavailable"
  | Invalid_registry detail -> "invalid V1 relay access registry: " ^ detail
  | Noncanonical_registry -> "V1 relay access registry is not canonical"
  | Registry_too_large ->
      "V1 relay access registry exceeds its configured limit"
  | Io_error { path; operation; message } ->
      Printf.sprintf "V1 relay access %s %s: %s" operation path message
