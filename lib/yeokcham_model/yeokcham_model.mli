...
type canonical_decode_error =
  | Canonical_payload_error of Yeokcham_encoding.decode_error
  ...
  | Invalid_canonical_identity of Yeokcham_id.parse_error
...
type retention_reason =
  | User_pinned
  | Capsule_boundary of Yeokcham_id.Capsule_id.t
  | Release_boundary of Yeokcham_id.Release_id.t
  | Validation_passed of Yeokcham_id.Validation_id.t
  | Periodic_retention
  | Recent_window
  | Conflict_reference of Yeokcham_id.Conflict_id.t
...
module Snapshot : sig
  ...
  val id : t -> Yeokcham_id.Snapshot_id.t
...
type event_transition_error =
  | Event_parent_mismatch of {
      expected_parent : Yeokcham_id.Checkpoint_id.t;
      actual_parent : Yeokcham_id.Checkpoint_id.t;
    }
...
module Scratch_event : sig
  val create :
    parent:Yeokcham_id.Checkpoint_id.t ->
    operations:scratch_operation list ->
    observed_at:int64 ->
    source:observation_source ->
    scratch_event

  val id : scratch_event -> Yeokcham_id.Operation_id.t
  val parent : scratch_event -> Yeokcham_id.Checkpoint_id.t
...
module Checkpoint : sig
  val create :
    parent:Yeokcham_id.Checkpoint_id.t option ->
    snapshot:Snapshot.t ->
    event:Yeokcham_id.Operation_id.t option ->
    created_at:int64 ->
  ...
  val id : checkpoint -> Yeokcham_id.Checkpoint_id.t
  val parent : checkpoint -> Yeokcham_id.Checkpoint_id.t option
  val snapshot : checkpoint -> Snapshot.t
  val event : checkpoint -> Yeokcham_id.Operation_id.t option
...
type repository_error =
  | Snapshot_identity_mismatch of {
      supplied : Yeokcham_id.Snapshot_id.t;
      computed : Yeokcham_id.Snapshot_id.t;
    }
  | Event_identity_mismatch of {
      supplied : Yeokcham_id.Operation_id.t;
      computed : Yeokcham_id.Operation_id.t;
    }
  | Checkpoint_identity_mismatch of {
      supplied : Yeokcham_id.Checkpoint_id.t;
      computed : Yeokcham_id.Checkpoint_id.t;
    }
  | Conflicting_snapshot of Yeokcham_id.Snapshot_id.t
  | Conflicting_event of Yeokcham_id.Operation_id.t
  | Conflicting_checkpoint of Yeokcham_id.Checkpoint_id.t
  | Missing_snapshot of Yeokcham_id.Snapshot_id.t
  | Missing_event of Yeokcham_id.Operation_id.t
  | Missing_checkpoint of Yeokcham_id.Checkpoint_id.t
  | Event_parent_missing of Yeokcham_id.Checkpoint_id.t
  | Incoherent_checkpoint of Yeokcham_id.Checkpoint_id.t
  | Replay_operation_rejected of replay_error
  | Target_not_descended_from of {
      ancestor : Yeokcham_id.Checkpoint_id.t;
      target : Yeokcham_id.Checkpoint_id.t;
    }
...
module Repository : sig
  ...
  val empty : t
  val scratch_head : t -> Yeokcham_id.Checkpoint_id.t option
  val find_snapshot : t -> Yeokcham_id.Snapshot_id.t -> Snapshot.t option
  val find_event : t -> Yeokcham_id.Operation_id.t -> scratch_event option
  val find_checkpoint : t -> Yeokcham_id.Checkpoint_id.t -> checkpoint option

  val insert_snapshot :
    t ->
    id:Yeokcham_id.Snapshot_id.t ->
    Snapshot.t ->
  ...
  val insert_checkpoint :
    t ->
    id:Yeokcham_id.Checkpoint_id.t ->
    checkpoint ->
    (t, repository_error) result
