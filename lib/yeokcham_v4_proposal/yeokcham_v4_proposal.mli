(** Parser-free, ephemeral assistance for an existing open V4 decision.
...
type provenance = {
  ...
  current_baseline : Yeokcham_v4_model.Snapshot_id.t;
  left_revision : Yeokcham_v4_model.Revision_id.t;
  left_base : Yeokcham_v4_model.Snapshot_id.t;
  left_result : Yeokcham_v4_model.Snapshot_id.t;
  right_revision : Yeokcham_v4_model.Revision_id.t;
  right_base : Yeokcham_v4_model.Snapshot_id.t;
  right_result : Yeokcham_v4_model.Snapshot_id.t;
}
...
type refusal =
  ...
  | Stale_base of {
      proposal_base : Yeokcham_v4_model.Snapshot_id.t;
      current_baseline : Yeokcham_v4_model.Snapshot_id.t;
    }
  | Conflicting_paths of Yeokcham_v4_model.Path.t list
...
type tree_error = Duplicate_path of Yeokcham_v4_model.Path.t

val tree_of_entries :
  (Yeokcham_v4_model.Path.t * entry) list -> (tree, tree_error) result

val classify :
  decision:Yeokcham_v4_model.Decision_id.t ->
  current_baseline:Yeokcham_v4_model.Snapshot_id.t ->
  left:Yeokcham_v4_model.change_revision ->
  right:Yeokcham_v4_model.change_revision ->
  base:tree ->
  left_tree:tree ->
  right_tree:tree ->
  t
...
val selected :
  t -> (Yeokcham_v4_model.Path.t * source * entry option) list option
