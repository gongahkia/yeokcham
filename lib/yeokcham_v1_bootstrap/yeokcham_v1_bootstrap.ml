...
let signature_domain = "yeokcham:v1:bootstrap-basis:1\000"
let portable_draft_id = "bootstrap-v1"
let portable_draft_title = "portable shared history"
let ( let* ) = Result.bind

let error_to_string = function
  | Invalid_basis detail -> "invalid V1 bootstrap basis: " ^ detail
  | Noncanonical_bytes -> "V1 bootstrap basis is not canonically encoded"
...
let exact_array name length = function
  ...
  | Encoding.Array _ ->
      Error (Invalid_basis (name ^ " has the wrong field count"))
  | Encoding.Integer _ | Encoding.Bytes _ | Encoding.Text _ | Encoding.Map _
  | Encoding.Bool _ | Encoding.Null ->
      Error (Invalid_basis (name ^ " must be an array"))
...
let encode basis =
  let unsigned =
    unsigned_bytes ~repository:basis.repository_value
      ~manifest:basis.manifest_value ~state:basis.state_value
      ~publisher:basis.publisher_value ~certificate:basis.certificate_value
    |> Result.get_ok
  in
  Encoding.array
    [
      Encoding.decode unsigned |> Result.get_ok;
      Encoding.bytes basis.signature_value;
    ]
...
let named_snapshots state =
  let from_changes =
    ...
    |> List.concat_map (fun change ->
        ...
        |> List.concat_map (fun revision ->
            [ revision.Model.base_snapshot; revision.Model.result_snapshot ]))
  in
  ...
  in
  let from_deliveries =
    state.Model.state_deliveries
    |> List.map (fun delivery -> delivery.Model.delivery_snapshot)
  in
  deduplicate_snapshots
    ((state.Model.state_baseline :: from_changes)
    @ from_resolutions @ from_deliveries)
...
let portable_state_of_project project =
  ...
  let state_checkpoints =
    List.map
      (fun checkpoint_snapshot -> Model.{ checkpoint_snapshot })
      snapshots
  in
  let state = { provisional with Model.state_checkpoints } in
  let* _ =
    Model.import state |> Result.map_error (fun error -> Model_error error)
  in
  Ok state
...
let valid_portable_state state =
  ...
  let active_drafts =
    List.filter
      (fun draft -> draft.Model.state = Model.Active)
      state.Model.state_drafts
  in
  ...
  if state.Model.state_pins <> [] || state.Model.state_usernames <> [] then
    ...
    | [ draft ]
      when Model.Draft_id.equal draft.Model.draft_id expected_draft
           ...
           && Model.Snapshot_id.equal draft.Model.latest_checkpoint
                state.Model.state_baseline
           && draft.Model.shared_change = None
           && List.for_all
                (fun change -> change.Model.source_draft = None)
                state.Model.state_changes ->
        Ok ()
...
let decode bytes =
  let* value =
    ...
    |> Result.map_error (fun error ->
        Invalid_basis (Encoding.decode_error_to_string error))
  in
      ...
      | [
       version; repository; manifest; state; publisher; certificate; algorithm;
          ...
          if not (Int64.equal version schema_version) then
            ...
            let* repository =
              text_field "bootstrap basis repository" repository
            in
            let* repository =
              Trust.Repository_id.of_string repository
              |> Result.map_error (fun detail -> Invalid_basis detail)
            in
            let* manifest = text_field "bootstrap basis manifest" manifest in
            let* () =
              if Transport.valid_digest manifest then Ok ()
              else
                Error (Invalid_basis "bootstrap basis manifest is not a digest")
            in
            let* state = bytes_field "bootstrap basis state" state in
            let* state =
              Record.decode_state state
              |> Result.map_error (fun error -> Record_error error)
            in
            let* () = valid_portable_state state in
            let* publisher = text_field "bootstrap basis publisher" publisher in
            let* publisher =
              Model.Device_id.of_string publisher
              |> Result.map_error (fun error -> Model_error error)
            in
            let* certificate =
              text_field "bootstrap basis certificate" certificate
            in
            let* () =
              if Transport.valid_digest certificate then Ok ()
              else
                Error
                  (Invalid_basis "bootstrap basis certificate is not a digest")
            in
            let* algorithm = text_field "bootstrap basis algorithm" algorithm in
            if not (String.equal algorithm Trust.algorithm) then
              Error (Invalid_basis "unsupported bootstrap basis algorithm")
            else
              let* canonical_unsigned =
                unsigned_bytes ~repository ~manifest ~state ~publisher
                  ~certificate
              in
              let canonical =
                Encoding.array
                ...
                Ok
                  {
                    ...
                    id_value = Transport.sha256 bytes;
                  }
      | _ -> assert false)
  | _ -> assert false
...
let require_state_records records state =
        ...
        let rec require_revisions = function
          ...
          | revision :: remaining ->
              ...
              if List.length matching = 1 then require_revisions remaining
                ...
                Error
                  (Invalid_basis
                     "shared projection revision lacks its signed record")
        in
        ...
        let matching =
          List.filter
            (fun signed ->
              Model.Revision_id.equal
                (Trust.signed_revision_id signed)
                resolution.Model.replacement_revision.Model.revision
              && Trust.signed_revision_value signed
                 = resolution.Model.replacement_revision
              && Trust.signed_revision_resolution signed
                 = Some resolution.Model.resolved_decision)
            records
        in
        if List.length matching = 1 then require_resolutions rest
        else
          Error (Invalid_basis "resolution projection lacks its signed record")
  in
  ...
  let rec require_deliveries = function
    ...
    | delivery :: rest ->
        if
          List.for_all
            (fun id -> List.exists (Model.Revision_id.equal id) all_ids)
            delivery.Model.included
        then require_deliveries rest
        else
          Error
            (Invalid_basis "delivery names a revision absent from the package")
  in
...
let create ~source ~destination ~project ~authority ~revisions ~authorizations
  ...
  if not (Trust.device_equal signing_device publisher) then
    Error
      (Invalid_basis "signing capability does not match bootstrap publisher")
  else
    let* unsigned =
      unsigned_bytes
        ~repository:(Trust.repository (Trust.authority_membership authority))
        ~manifest:manifest_value ~state
    ...
    let provisional =
      {
        ...
        manifest_value;
        state_value = state;
        publisher_value = Trust.device_id publisher;
        certificate_value = certificate;
        signature_value = signature;
        id_value = "";
      }
...
let verify ~repository ~package ~bytes =
  ...
  if not (Trust.Repository_id.equal repository basis.repository_value) then
    ...
    if
      not
        (String.equal basis.manifest_value
           (Transport.sha256 (Package.artifact_manifest artifact)))
    then Error (Invalid_basis "bootstrap basis manifest does not match package")
    else
      let* authority =
        Package.inspect_authority ~package
        |> Result.map_error (fun error -> Package_error error)
      in
      if
        not
          (Trust.Repository_id.equal repository
             (Trust.repository (Trust.authority_membership authority)))
      then
        ...
        let certificate =
          ...
          |> List.find_opt (fun certificate ->
              String.equal
                ...
                basis.certificate_value)
        in
        let* certificate =
          match certificate with
          | Some certificate -> Ok certificate
          | None ->
              Error (Invalid_basis "bootstrap publisher certificate is absent")
        in
        let publisher = Trust.certificate_subject certificate in
        if
          not
            (Model.Device_id.equal
               (Trust.device_id publisher)
               basis.publisher_value)
        then
          ...
          not
            (List.exists
               (fun epoch ->
                 Trust.authority_device_active authority ~epoch publisher)
               (Trust.authority_heads authority))
        then
          Error
            (Invalid_basis
               "bootstrap publisher is not active at a current authority head")
        else
          let* unsigned =
            unsigned_bytes ~repository:basis.repository_value
              ~manifest:basis.manifest_value ~state:basis.state_value
              ~publisher:basis.publisher_value
          ...
          in
          let* records =
            Package.validate_with_authority ~package ~authority
            |> Result.map_error (fun error -> Package_error error)
          in
          let* () =
            Package.validate_snapshot_closure ~package
              ~snapshots:(named_snapshots basis.state_value)
            |> Result.map_error (fun error -> Package_error error)
          in
          let* () =
            require_state_records (Package.revisions records) basis.state_value
          in
          Ok
            {
              verified_basis = basis;
              verified_package = artifact;
...
let import ~destination verified ~creator ~username ~initial_draft ~title
  ...
  let* certificate =
    ...
    | None ->
        Error
          (Invalid_basis "local certificate is absent from bootstrap authority")
  in
  if
    not
      (Model.Device_id.equal
         ...
         creator)
  then Error (Invalid_basis "local certificate does not match bootstrap device")
  else if
    not
      (List.exists
         (fun epoch ->
