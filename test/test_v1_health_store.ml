...
let with_root run =
  let root = Filename.temp_file "v1-health-plan-store-" "" in
  Unix.unlink root;
  ...
  with_root (fun root ->
      ...
      | Error error ->
          Alcotest.(check bool)
            ...
            (String.starts_with
               ~prefix:"V1 repair plan path contains different bytes: "
      ...
      | Error error ->
          Alcotest.(check bool)
            ...
            (String.starts_with ~prefix:"invalid V1 repair source locator: "
               (Plan_store.error_to_string error))
      | Ok _ -> Alcotest.fail "corrupt plan decoded")

let () =
  Alcotest.run "V1 health plan store"
    [
      ( "persistence",
        [
