...
let canonical_success_fixture_round_trips () =
  ...
  in
  let expected =
    read_file (fixture_path "v4/cli-envelope-v1.json") |> String.trim
  in
...
let generic_completed_fixture_round_trips () =
  ...
  in
  let expected =
    read_file (fixture_path "v4/cli-completed-envelope-v1.json") |> String.trim
  in
  Alcotest.(check string)
