...
let watch_records_a_checkpoint_after_quiet_edits () =
  with_directory "yeokcham-v1-watch-" (fun root ->
      ...
      let log = Filename.temp_file "yeokcham-v1-watch-log-" "" in
      let log_fd =
...
let watch_debounces_a_rapid_edit_storm () =
  with_directory "yeokcham-v1-watch-storm-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      ...
      in
      require_success "init" status errors;
      let log = Filename.temp_file "yeokcham-v1-watch-storm-log-" "" in
      let log_fd =
        Unix.openfile log [ Unix.O_WRONLY; Unix.O_CREAT; Unix.O_TRUNC ] 0o600
      in
      let exe = executable () in
...
let watch_exits_after_permanent_root_loss () =
  with_directory "yeokcham-v1-watch-root-loss-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let _output, errors, status =
        run
          [
      ...
      in
      require_success "init" status errors;
      let log = Filename.temp_file "yeokcham-v1-watch-root-loss-log-" "" in
      let log_fd =
        Unix.openfile log [ Unix.O_WRONLY; Unix.O_CREAT; Unix.O_TRUNC ] 0o600
      in
      let exe = executable () in
...
let () =
  Alcotest.run "V1 foreground watch"
    [
      ( "capture",
        [
