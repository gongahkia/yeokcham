...
let with_repository callback =
  let root = Filename.temp_file "yeokcham-v1-custody-" "" in
  Unix.unlink root;
  ...
  with_repository (fun root ->
      ...
      | Error error ->
          Alcotest.(check string)
            ...
            "V1 local custody profile already exists"
...
let denied_signer_cannot_create_a_record () =
  ...
  | Error error ->
      Alcotest.(check string)
        ...
        "V1 signer failed: user denied signing"
...
let with_ssh_agent root callback =
  let socket =
    Filename.temp_file ~temp_dir:"/tmp" "yeokcham-v1-agent-" ".sock"
  in
  Unix.unlink socket;
  ...
  with_repository (fun root ->
      with_ssh_agent root (fun public_key_path ->
          ...
          Alcotest.(check (result string string))
            ...
            (Error
               "V1 signer is non-exportable and cannot provide private key \
                bytes")
...
let pkcs11_signs_every_v1_action () =
  ...
  | Some module_path ->
      with_repository (fun root ->
          let pin = "1234" in
          let token_label = "yeokcham-v1-test" in
          let key_id =
            "yeokcham-v1-custody-" ^ string_of_int (Unix.getpid ())
          in
          let device_id =
            Custody.create_pkcs11_with_pin ~root ~module_path ~token_label
              ~key_label:
                ("yeokcham-v1-test-key-" ^ string_of_int (Unix.getpid ()))
              ~key_id ~pin
            |> require_ok Custody.error_to_string
          in
          let capability =
          ...
          | Error error ->
              Alcotest.(check string)
                ...
                "V1 signer failed: V1 PKCS#11 token is locked or denied signing"
                (Trust.error_to_string error)
          ...
          | Error error ->
              Alcotest.(check string)
                ...
                "V1 PKCS#11 provider is unavailable: module or token could not \
                 be opened"
                (Custody.error_to_string error)
          | Ok _ -> Alcotest.fail "unavailable PKCS#11 provider was attached")

let () =
  Alcotest.run "V1 custody"
    [
      ( "profiles",
        [
