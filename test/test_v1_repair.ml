...
let initialized_pair run =
  with_directory "v1-repair-target-" (fun root ->
      with_directory "v1-repair-backup-" (fun backup ->
...
let stage_exact_snapshot_in_quarantine root status =
  ...
  let state =
    {
      Store.id = loaded.V1_store.object_id;
      object_type = Envelope.V1_project_state;
      stored_bytes =
        (Unix.stat (Store.object_path store loaded.V1_store.object_id))
          .Unix.st_size;
    }
  in
  let object_ =
  ...
  in
  let plan =
    Gc.classify ~state_head:loaded.V1_store.object_id
  ...
  Unix.mkdir gc 0o700;
  let directory =
    Filename.concat gc ("v1-gc-" ^ Gc.transaction_id transaction)
  in
...
let[@warning "-4"] changed_state_head_refuses_stale_approval () =
  initialized_pair (fun root backup status ->
      ...
      in
      let project =
        Model.register_username loaded.V1_store.project
      ...
      in
      ignore
        (V1_store.save repository ~expected:loaded.V1_store.head ~project
...
let bootstrap_artifact_source_is_verified_without_bootstrap () =
  with_directory "v1-repair-bootstrap-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
...
let configured_relay_source_is_verified_without_receipt () =
  with_https_relay (fun client project url ->
      with_directory "v1-repair-relay-target-" (fun root ->
          write_file root "main.ml" "let version = 1\n";
