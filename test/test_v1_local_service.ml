...
let init_captures_the_initial_tree_and_save_observes_no_change () =
  with_directory "yeokcham-v1-service-init-" (fun root ->
...
let username_registration_persists_across_a_repository_reload () =
  with_directory "yeokcham-v1-service-usernames-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
...
let changed_save_creates_a_new_checkpoint () =
  with_directory "yeokcham-v1-service-save-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
...
let status_warns_about_uncaptured_edits () =
  with_directory "yeokcham-v1-service-uncaptured-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      ignore (initialize root);
      write_file root "main.ml" "let version = 2\n";
...
let working_tree_inspection_reports_exact_stable_entries_without_state_change ()
    =
  let open Service in
  with_directory "yeokcham-v1-service-changes-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
...
let compact_drops_extra_saves_and_keeps_a_pin () =
  with_directory "yeokcham-v1-service-compact-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      ignore (initialize root);
      write_file root "main.ml" "let version = 2\n";
      let first =
...
let new_draft_closes_the_previous_draft_without_losing_saved_state () =
  with_directory "yeokcham-v1-service-draft-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let initial = initialize root in
      let next =
...
let restore_materializes_a_prior_checkpoint_without_touching_the_project () =
  with_directory "yeokcham-v1-service-restore-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let initial = initialize root in
      write_file root "main.ml" "let version = 2\n";
      ignore (Service.save ~root |> require_ok Service.error_to_string);
...
let restore_rejects_a_nonempty_destination () =
  with_directory "yeokcham-v1-service-restore-failure-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let initial = initialize root in
...
let v1_capture_excludes_git_metadata () =
  with_directory "yeokcham-v1-service-git-metadata-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
...
let in_place_restore_retains_and_recovers_unsaved_bytes () =
  with_directory "yeokcham-v1-service-in-place-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let initial = initialize root in
...
let restore_proof_survives_journal_pruning_until_explicit_forget () =
  with_directory "yeokcham-v1-service-restore-proof-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let initial = initialize root in
      write_file root "main.ml" "let unsaved = 2\n";
      let restored =
...
let legacy_published_journal_remains_a_root_until_explicit_retain () =
  with_directory "yeokcham-v1-service-legacy-restore-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let initial = initialize root in
      write_file root "main.ml" "let version = 2\n";
      let saved =
...
let corrupt_restore_proof_blocks_compaction_before_state_change () =
  with_directory "yeokcham-v1-service-corrupt-proof-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let initial = initialize root in
      write_file root "main.ml" "let unsaved = 2\n";
      let restored =
      ...
      let path =
        Filename.concat
          ...
          ("v1-restore-proof-" ^ restored.Service.restore_operation ^ ".cbor")
      in
      let before =
...
let missing_restore_proof_object_blocks_compaction_before_state_change () =
  with_directory "yeokcham-v1-service-missing-proof-object-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let initial = initialize root in
      let repository =
        Store.open_repository ~root |> require_ok Store.error_to_string
...
let in_place_restore_preserves_repository_metadata () =
  with_directory "yeokcham-v1-service-in-place-metadata-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let git = Filename.concat root ".git" in
      Unix.mkdir git 0o700;
...
let interrupted_applying_restore_is_resumable () =
  with_directory "yeokcham-v1-service-resume-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let initial = initialize root in
      write_file root "main.ml" "let version = 2\n";
      let saved =
...
let share_records_a_shared_change_and_amend_appends_a_revision () =
  with_directory "yeokcham-v1-service-share-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      ignore (initialize root);
      write_file root "main.ml" "let version = 2\n";
      let shared =
...
let share_without_a_tree_delta_fails () =
  with_directory "yeokcham-v1-service-share-empty-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      ignore (initialize root);
      match
        Service.share ~root ~change:(change "change-a")
...
let overlapping_shared_drafts_are_a_decision () =
  with_directory "yeokcham-v1-service-overlap-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let initial = initialize root in
      write_file root "main.ml" "let version = 2\n";
      ignore
...
let isolated_resolve_does_not_rewrite_the_live_tree () =
  with_directory "yeokcham-v1-service-isolated-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      ignore (initialize root);
      write_file root "main.ml" "let version = 2\n";
      ignore
...
let isolated_inspection_compares_exact_candidate_snapshots () =
  with_directory "yeokcham-v1-service-inspect-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      ignore (initialize root);
      write_file root "main.ml" "let version = 2\n";
      ignore
...
let exact_proposal_materializes_without_accepting_or_rewriting () =
  with_directory "yeokcham-v1-service-proposal-ready-" (fun root ->
      with_external_directory "yeokcham-v1-proposal-output-" (fun output_root ->
          write_file root "main.ml" "let version = 1\n";
...
let proposal_refusal_and_missing_closure_leave_every_destination_untouched () =
  with_directory "yeokcham-v1-service-proposal-refusal-" (fun root ->
      with_external_directory "yeokcham-v1-proposal-output-" (fun output_root ->
          write_file root "main.ml" "let version = 1\n";
          ignore (initialize root);
          write_file root "main.ml" "let version = 2\n";
...
let proposal_pair_overview_lists_every_competing_pair_once () =
  with_directory "yeokcham-v1-service-proposal-pairs-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      ignore (initialize root);
      write_file root "main.ml" "let version = 2\n";
      ignore
...
let exact_proposal_preserves_binary_bytes_and_symlink_targets () =
  with_directory "yeokcham-v1-service-proposal-bytes-" (fun root ->
      with_external_directory "yeokcham-v1-proposal-output-" (fun output_root ->
...
let withdraw_protects_the_active_shared_change () =
  with_directory "yeokcham-v1-service-withdraw-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      ignore (initialize root);
      write_file root "main.ml" "let version = 2\n";
      ignore
...
let resolve_clears_the_open_decision () =
  with_directory "yeokcham-v1-service-resolve-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      ignore (initialize root);
      write_file root "main.ml" "let version = 2\n";
      ignore
...
let deliver_requires_a_decision_free_shared_draft () =
  with_directory "yeokcham-v1-service-deliver-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      ignore (initialize root);
      write_file root "main.ml" "let version = 2\n";
      ignore
...
let signed_offline_receive_is_atomic_and_preserves_the_live_tree () =
  with_directory "yeokcham-v1-signed-receive-" (fun root ->
...
let administrator_enrollment_persists_a_public_member_and_local_username () =
  with_directory "yeokcham-v1-enrollment-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let administrator_capability = signing_capability 'a' in
      let administrator = trust_device administrator_capability in
      let recovery_capability = signing_capability 'r' in
...
let authority_initialized_repositories_exchange_verified_epoch_bound_work () =
  with_directory "yeokcham-v1-authority-source-" (fun parent ->
...
let signed_transport_receive_is_atomic_and_idempotent () =
  with_directory "yeokcham-v1-transport-receive-" (fun parent ->
      let source = Filename.concat parent "source" in
      let destination = Filename.concat parent "destination" in
      Unix.mkdir source 0o700;
      Unix.mkdir destination 0o700;
      ...
      | Error error ->
          Alcotest.(check string)
            ...
            "invalid V1 transport publication: publication manifest does not \
      ...
      let received =
        Service.receive_transport_batch ~root:destination ~remote:"team"
          ~cursor:
            (Some
               (Yeokcham_v1_transport.publication_id
                  outbound.Service.outbound_publication))
          [ arrival ]
        |> require_ok Service.error_to_string
      in
...
let signed_resolution_is_persisted_with_its_decision_purpose () =
  with_directory "yeokcham-v1-signed-resolution-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let administrator_capability = signing_capability 'a' in
      let administrator = trust_device administrator_capability in
      let recovery_capability = signing_capability 'r' in
...
let authority_lifecycle_revokes_rotates_and_recovers_a_replacement_device () =
  with_directory "yeokcham-v1-authority-lifecycle-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let administrator_capability = signing_capability 'a' in
      let administrator = trust_device administrator_capability in
      let recovery_capability = signing_capability 'r' in
...
let normal_device_rotation_replaces_the_local_signing_identity_atomically () =
  with_directory "yeokcham-v1-device-rotation-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let administrator_capability = signing_capability 'a' in
      let administrator = trust_device administrator_capability in
      let recovery_capability = signing_capability 'r' in
...
let denied_external_rotation_leaves_authority_state_unchanged () =
  with_directory "yeokcham-v1-denied-rotation-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let administrator_capability = signing_capability 'a' in
      let administrator = trust_device administrator_capability in
      let recovery_capability = signing_capability 'r' in
      ...
      | Error error ->
          Alcotest.(check string)
            ...
            "V1 signer failed: user denied rotation"
            (Service.error_to_string error)
...
let authority_forks_require_named_branch_actions_and_explicit_reconciliation ()
    =
  with_directory "yeokcham-v1-authority-fork-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let administrator_capability = signing_capability 'a' in
      let administrator = trust_device administrator_capability in
      let recovery_capability = signing_capability 'r' in
      ...
      | Error error ->
          Alcotest.(check string)
            ...
            "invalid V1 authority epoch: reconciliation parent heads must be \
...
let late_package_review_retains_feed_forks_across_retry_before_adoption () =
  with_directory "yeokcham-v1-late-package-review-" (fun parent ->
      let source = Filename.concat parent "source" in
      let destination = Filename.concat parent "destination" in
      Unix.mkdir source 0o700;
      Unix.mkdir destination 0o700;
      ...
      Yeokcham_v1_package.create_with_authority
      ...
      |> require_ok Yeokcham_v1_package.error_to_string;
      ...
      ( "saved work",
        [
          ...
          Alcotest.test_case "capture excludes Git metadata" `Quick
            v1_capture_excludes_git_metadata;
          Alcotest.test_case
