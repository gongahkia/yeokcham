...
let () =
  Printf.printf "V2 restore-recovery property base seed: %d\n%!" base_seed
...
let with_repository run =
  let root = Filename.temp_file "yeokcham-v2-restore-recovery-property-" "" in
  Unix.unlink root;
...
let generated_interrupted_restores_recover_and_publish =
  QCheck2.Test.make ~count:60
    ~name:
      "V2 authenticated restore recovery publishes generated target after \
       interruption"
      ...
      if String.equal target_bytes observed_bytes then true
        ...
        with_repository (fun root bootstrap_repository scratch ->
            ...
            | Ok target_snapshot -> (
                ...
                | Ok (Scratch.Published target) -> (
                    write_file work observed_bytes;
                    match
                      Preparation.prepare ~root ~bootstrap_repository
                        ~target_event_id:target.Scratch.event_id ~operation_id
                        ~safety_snapshot_nonce:(nonce '3')
                        ...
                        | Ok journal_store -> (
                            ...
                            | Error
                                ...
                                | Ok result -> (
                                    ...
                                    | Error _ -> false
                                    | Ok actual ->
                                        Journal.phase (Recovery.journal result)
                                        = Journal.Published
...
let () =
  Alcotest.run "V2 authenticated restore recovery properties"
    [
      ( "property",
        [
          QCheck_alcotest.to_alcotest ~speed_level:`Quick
            ~rand:(state_for "interrupted-publish")
            generated_interrupted_restores_recover_and_publish;
        ] );
    ]
