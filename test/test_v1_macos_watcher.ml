...
let observes_create_modify_delete_and_rename () =
  with_directory "yeokcham-v1-macos-fsevents-" (fun root ->
...
let metadata_only_activity_never_narrows_to_a_source_path () =
  with_directory "yeokcham-v1-macos-fsevents-metadata-" (fun root ->
...
let close_releases_the_source () =
  with_directory "yeokcham-v1-macos-fsevents-close-" (fun root ->
      let watcher = Macos_watcher.start ~root |> require_ok in
...
let root_loss_requests_restart_without_a_path_guess () =
  with_directory "yeokcham-v1-macos-fsevents-root-loss-" (fun root ->
...
let rapid_rename_storm_remains_advisory () =
  with_directory "yeokcham-v1-macos-fsevents-storm-" (fun root ->
...
let () =
  Alcotest.run "V1 macOS FSEvents watcher"
    [
      ( "native source",
        [
