...
let with_runtime run_test =
  with_directory "yeokcham-v4-runtime-repository-" (fun root ->
      with_directory "yeokcham-v4-runtime-state-" (fun runtime ->
          let signer = Filename.concat runtime "signers" in
          Unix.mkdir signer 0o700;
          with_environment "XDG_RUNTIME_DIR" runtime (fun () ->
              with_environment "YEOKCHAM_V4_TEST_SIGNER_DIRECTORY" signer
  ...
  with_runtime (fun ~root ~runtime ->
      ...
      Fun.protect
        ...
        (fun () ->
          ...
          let state_root = Filename.concat runtime "yeokcham-v4" in
...
let explicit_sync_failure_does_not_capture_or_materialize () =
  with_runtime (fun ~root ~runtime:_ ->
      with_environment "YEOKCHAM_V4_TEST_TRANSPORT" "1" (fun () ->
          with_environment "YEOKCHAM_V4_TEST_TRANSPORT_TOKEN"
...
let runtime_requires_private_xdg_directory () =
  with_directory "yeokcham-v4-runtime-no-xdg-" (fun root ->
      ...
      with_directory "yeokcham-v4-runtime-public-" (fun runtime ->
...
let () =
  Alcotest.run "V4 Linux background runtime"
    [
      ( "lifecycle",
        [
