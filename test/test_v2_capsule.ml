...
let split_requires_nonempty_right_partition () =
  let source = source_snapshot () in
  let target = target_snapshot () in
  let proposal =
    Capsule.propose ~from:source ~to_:target
  ...
  in
  let capsule =
    Capsule.make_capsule ~id:(capsule_id 'n') ~title:"nonempty partition"
      ~description:"right partition remains meaningful" ~created_at:19L
    |> require_ok Capsule.error_to_string
  in
  let source_link = link source 's' in
  let target_link = link target 't' in
  let revision =
    Capsule.make_initial_revision ~capsule ~capsule_ref:(opaque 'n')
      ~declared_base:source_link ~declared_base_snapshot:source
      ~expected_result:target_link ~selected
      ~source_boundary:
        { Capsule.source_snapshot = source_link; target_snapshot = target_link }
    |> require_ok Capsule.error_to_string
  in
  (match Capsule.plan_split ~base:source revision ~left_indices:indices with
  | Error error -> (
      match error with
      | Capsule.Split_empty_right_partition -> ()
      | error ->
          Alcotest.fail
            ("all-operation split returned the wrong error: "
            ^ Capsule.split_error_to_string error))
  | Ok _ ->
      Alcotest.fail
        "all-operation split unexpectedly produced an empty right partition")
  [@warning "-4"]
    ...
    [
      ( "unit",
        [
          ...
          Alcotest.test_case "split requires a nonempty right partition" `Quick
            split_requires_nonempty_right_partition;
