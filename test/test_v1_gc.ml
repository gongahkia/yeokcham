...
let pure_classification_keeps_reachable_objects_and_unsupported_types () =
  ...
  let plan =
    Gc.classify ~state_head
      ~objects:
        [
          info 'a' Envelope.V1_project_state 10;
...
let transaction_record_has_stable_canonical_bytes () =
  let plan =
    Gc.classify ~state_head:(id 'a')
      ~objects:
        [ info 'a' Envelope.V1_project_state 10; info 'b' Envelope.Content 20 ]
      ~reachable:[]
    |> require_ok Gc.error_to_string
  in
  let transaction = Gc.make_transaction plan |> require_ok Gc.error_to_string in
  let expected =
    Golden.read_lower_hex_file (golden_path "v1/gc-transaction-v1.cbor.hex")
    |> require_ok Fun.id
  in
  let actual =
...
let local_quarantine_restore_and_purge_are_explicit () =
  with_directory "yeokcham-v1-gc-" (fun root ->
...
let named_manifest_and_chunk_closure_is_not_collectable () =
  with_directory "yeokcham-v1-gc-manifest-" (fun root ->
...
let shared_snapshot_closure_has_an_explicit_retention_reason () =
  with_directory "yeokcham-v1-gc-shared-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      ignore (initialize root);
...
let initialized_empty_worktree_has_no_collectible_objects () =
  with_directory "yeokcham-v1-gc-empty-" (fun root ->
      ignore (initialize root);
      let plan = Gc.plan ~root |> require_ok Gc.error_to_string in
      Alcotest.(check int)
...
let purge_recovers_after_a_durable_marker_before_unlink () =
  with_directory "yeokcham-v1-gc-purge-recovery-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      ignore (initialize root);
      let repository =
        V1_store.open_repository ~root |> require_ok V1_store.error_to_string
      ...
      let transaction_directory =
        Filename.concat
          ...
          ("v1-gc-" ^ Gc.transaction_id transaction)
      in
...
let missing_reachable_closure_blocks_planning_without_quarantine_mutation () =
  with_directory "yeokcham-v1-gc-missing-closure-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      let status = initialize root in
      let repository =
        V1_store.open_repository ~root |> require_ok V1_store.error_to_string
...
let state_head_change_after_quarantine_blocks_purge () =
  with_directory "yeokcham-v1-gc-stale-head-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      ignore (initialize root);
      write_file root "main.ml" "let version = 2\n";
      ...
      let current =
        V1_store.load repository |> require_ok V1_store.error_to_string
      in
      V1_store.save repository ~expected:current.V1_store.head
        ~project:before_compaction.V1_store.project
...
let corrupted_object_blocks_planning_without_quarantine_mutation () =
  with_directory "yeokcham-v1-gc-corrupt-" (fun root ->
      write_file root "main.ml" "let version = 1\n";
      ignore (initialize root);
      let repository =
        V1_store.open_repository ~root |> require_ok V1_store.error_to_string
...
let () =
  Alcotest.run "V1 garbage collection"
    [
      ( "planner",
        [
