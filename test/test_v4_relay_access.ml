...
let canonical_registry_fixture () =
  let encoded = read_golden "v4/relay-access-registry-v1.cbor.hex" in
  ...
  Access.authorize ~now:1_999L
    ~secret:("v4ra1_" ^ String.make 64 '0')
...
let corrupt_persistent_registry_fails_closed () =
  with_directory "yeokcham-v4-relay-access-" (fun root ->
