...
let custody_profile_is_canonical_and_local () =
  with_repository (fun root ->
      ...
      let device =
        Trust.device_of_public_key public_key |> require_ok Trust.error_to_string
      in
      let profile =
        { Custody.device = Trust.device_id device; provider = Custody.Ssh_agent { public_key } }
      in
      ...
      Alcotest.(check int) "profile mode" 0o600 ((Unix.stat path).Unix.st_perm land 0o777);
      let loaded = Custody.find ~root profile.Custody.device |> require_ok Custody.error_to_string in
      Alcotest.(check bool) "profile is local exact device" true
        (Model.Device_id.equal loaded.Custody.device profile.Custody.device);
      match Custody.save ~root profile with
      | Error error ->
          Alcotest.(check string) "profile overwrite refused"
            "V4 local custody profile already exists" (Custody.error_to_string error)
...
let denied_signer_cannot_create_a_record () =
  ...
  | Error error ->
      Alcotest.(check string) "user denial reaches the trust boundary"
        "V4 signer failed: user denied signing" (Trust.error_to_string error)
...
let wait_for_socket socket =
  let rec loop remaining =
    ...
    else if remaining = 0 then Alcotest.fail "ssh-agent did not create its socket"
    else (
      Unix.sleepf 0.02;
      loop (remaining - 1))
  in
...
let ssh_agent_signs_only_for_its_explicit_key () =
  with_repository (fun root ->
      with_ssh_agent root (fun public_key_path ->
          ...
          let signature =
            Trust.sign_detached capability ~domain:"yeokcham:test:agent:\000" "exact bytes"
            |> require_ok Trust.error_to_string
          in
          Trust.verify_detached ~device ~domain:"yeokcham:test:agent:\000"
            ~signature "exact bytes"
          |> require_ok Trust.error_to_string;
          Alcotest.(check (result string string)) "non-exportable agent key"
            (Error "V4 signer is non-exportable and cannot provide private key bytes")
...
let sample_revision author =
  Model.make_change_revision ~change:(change "custody-change")
    ~revision:(revision "custody-revision") ~parent:None ~author
    ~base:(snapshot "custody-base") ~result:(snapshot "custody-result")
    ~edits:
      [
        {
          Model.edit_path = Model.Path.of_components [ "main.ml" ] |> Result.get_ok;
...
let pkcs11_signs_every_v4_action () =
  ...
  | Some module_path ->
      with_repository (fun root ->
          ...
          | Error error ->
              Alcotest.(check string) "PKCS#11 capability is non-exportable"
          ...
          let signed =
            Trust.sign_revision_at authority ~epoch:(Trust.epoch_id epoch)
              ~certificate:(Trust.certificate_id root_certificate) capability revision
            |> require_ok Trust.error_to_string
          in
          ...
          let resolution =
            Trust.sign_resolution_at authority ~epoch:(Trust.epoch_id epoch)
              ~certificate:(Trust.certificate_id root_certificate) capability
              ~decision revision
            |> require_ok Trust.error_to_string
          in
          ignore resolution;
          Trust.make_authorization authority ~epoch:(Trust.epoch_id epoch)
            ...
            ~device:root_device ~revision:revision.Model.revision
