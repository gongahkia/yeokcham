...
let persistent_records_have_canonical_golden_bytes () =
  ...
  in
  let expected_basis =
    Golden.read_lower_hex_file (golden_path "v1/projection-basis-v1.cbor.hex")
    |> require_ok Fun.id
  in
  let expected_receipt =
    Golden.read_lower_hex_file
      (golden_path "v1/workspace-projection-receipt-v1.cbor.hex")
    |> require_ok Fun.id
  in
  Alcotest.(check string)
...
let with_root run =
  let root = Filename.temp_file "v1-workspace-" "" in
  Unix.unlink root;
