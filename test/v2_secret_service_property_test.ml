...
let with_root run =
  let root = Filename.temp_file "yeokcham-v2-secret-service-property-" "" in
  Unix.unlink root;
...
let runner stored
    ({ Secret_service.arguments; stdin; _ } : Secret_service.command) =
  match arguments with
  | "--user" :: "get-property" :: _ -> Ok (command_result 0 "b false\n")
  | "lookup" :: _ -> (
      match !stored with
      | None -> Ok (command_result 1 "")
      | Some value -> Ok (command_result 0 (value ^ "\n")))
  | "store" :: _ ->
      stored := Some stdin;
      Ok (command_result 0 "")
  | _ -> Error Secret_service.Spawn_failed
...
let enrollment_reopens =
  QCheck2.Test.make ~count:120
    ~name:
      "generated bootstrap IDs and Secret Service handles reopen one matching \
    ...
    (fun (repository_bytes, device_bytes, handle_bytes) ->
      match
        ( Model.Repository_id.of_bytes repository_bytes,
          ...
          Secret_service.key_handle_of_bytes handle_bytes )
      with
      | Ok repository_id, Ok device_id, Ok key_handle ->
          with_root (fun root ->
              ...
              | Ok _ -> (
                  ...
                  let service =
                    Secret_service.service ~runner:(runner stored)
                      ~secret_tool:"secret-tool" ~busctl:"busctl"
                  in
                  match
                    Secret_service.enroll ~service ~root ~repository_id
...
let () =
  Alcotest.run "V2 Linux Secret Service custody properties"
    [
      ( "property",
        [
          QCheck_alcotest.to_alcotest ~speed_level:`Quick
            ~rand:(state_for "secret-service-enrollment")
            enrollment_reopens;
        ] );
    ]
