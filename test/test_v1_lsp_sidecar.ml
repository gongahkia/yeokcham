...
module Sidecar = Yeokcham_v1_lsp_sidecar
module Config = Yeokcham_v1_semantic_config
...
let reports_observations_from_disposable_snapshots () =
  with_directory "yeokcham-v1-lsp-" (fun root ->
      let store = Store.init ~root |> require_ok Store.error_to_string in
      let capture = Filename.temp_file "yeokcham-v1-lsp-capture-" "" in
      Fun.protect
        ~finally:(fun () ->
...
let rejected_or_bad_server_output_preserves_repository () =
  with_directory "yeokcham-v1-lsp-state-" (fun root ->
      let store = Store.init ~root |> require_ok Store.error_to_string in
      let snapshots = snapshots root store in
...
let broader_sensitivity_does_not_require_matching_symbol_names () =
  with_directory "yeokcham-v1-lsp-granularity-" (fun root ->
      let store = Store.init ~root |> require_ok Store.error_to_string in
      let snapshots = snapshots root store in
...
let () =
  Alcotest.run "yeokcham v1 LSP sidecar"
    [
      ( "sidecar",
        [
