...
let create_hybrid_legacy root =
  ...
  let object_id = Store.put store (content_envelope "legacy object bytes") |> require_ok Store.error_to_string in
  ignore
...
let classification_is expected actual =
  Alcotest.(check string) "classification"
...
let classifications_are_explicit () =
  ...
  with_root (fun root ->
      ...
      let classification = Cutover.detect ~root |> require_ok Cutover.error_to_string in
      Alcotest.(check bool) "symlink root is mixed or unknown" true
  ...
  with_root (fun root ->
      ...
      let classification = Cutover.detect ~root |> require_ok Cutover.error_to_string in
      Alcotest.(check bool) "incomplete root is explicit" true
...
let tree_entries root initial =
  ...
  | Unix.S_DIR ->
      let own = if String.is_empty relative then [] else [ Directory (relative, stat.Unix.st_perm land 0o7777) ] in
      let children =
        Sys.readdir path |> Array.to_list |> List.sort String.compare
        |> List.concat_map (fun name ->
               let child = if String.is_empty relative then name else relative ^ "/" ^ name in
               collect (Filename.concat path name) child)
      in
      own @ children
  | Unix.S_REG -> [ File (relative, stat.Unix.st_perm land 0o7777, read_file path) ]
...
let archived_tree_is_byte_and_mode_preserving () =
  with_root (fun root ->
      ...
      let manifest_path =
        ...
        | Cutover.Already_archived _ -> Alcotest.fail "new archive was reported as already archived"
      in
      let golden =
        Golden.read_lower_hex_file "golden/legacy-archive-manifest-v1.cbor.hex"
        |> require_ok Fun.id
      in
      Alcotest.(check string) "canonical archive manifest golden" golden
        (read_file manifest_path);
      Alcotest.(check bool) "archive preserves bytes and modes" true
        (legacy_before = tree_entries root "legacy-before-v2");
      Alcotest.(check bool) "legacy metadata is relocated" false
        (Sys.file_exists (metadata_path root));
      let unconfirmed = Cutover.reset ~root ~archive_name:"legacy-before-v2" ~confirm:false in
      Alcotest.(check string) "reset requires confirmation"
      ...
      Alcotest.(check bool) "unconfirmed reset leaves no V2 root" false
        (Sys.file_exists (metadata_path root));
      let reset =
        Cutover.reset ~root ~archive_name:"legacy-before-v2" ~confirm:true
        |> require_ok Cutover.error_to_string
      in
      Alcotest.(check bool) "reset result" true
        (match reset with Cutover.Reset -> true | Cutover.Already_reset -> false);
      Cutover.detect ~root |> require_ok Cutover.error_to_string
      |> classification_is Cutover.V2;
      Alcotest.(check bool) "reset keeps archive untouched" true
        (legacy_before = tree_entries root "legacy-before-v2");
      let repeated =
        Cutover.reset ~root ~archive_name:"legacy-before-v2" ~confirm:true
        |> require_ok Cutover.error_to_string
      in
      Alcotest.(check bool) "repeated reset is idempotent" true
        (match repeated with Cutover.Reset -> false | Cutover.Already_reset -> true))
...
let tampered_archive_cannot_reset () =
  with_root (fun root ->
      ...
      match Cutover.reset ~root ~archive_name:"legacy-tampered" ~confirm:true with
      | Error _ -> ()
  ...
  with_root (fun root ->
      ...
      Alcotest.(check bool) "archive does not overwrite existing sibling" true
        (match outcome with
        | Ok _ -> false
        | Error error ->
...
let stable_seed name =
  ...
  String.iter (fun character -> value := !value * 65599 lxor Char.code character land max_int) name;
  !value

let state_for name = Random.State.make [| stable_seed name |]

let safe_legacy_trees_are_preserved =
  QCheck2.Test.make ~count:80 ~name:"generated legacy trees archive with exact bytes and modes"
    ...
    (fun contents ->
      with_root (fun root ->
          ...
          List.iteri
            (fun index content ->
              let path = Filename.concat (Filename.concat metadata "locks") (Printf.sprintf "entry-%d" index) in
    ...
    [
      ( "unit",
        [
          ...
          Alcotest.test_case "archive preserves bytes and modes before reset" `Quick
            archived_tree_is_byte_and_mode_preserving;
      ...
      ( "property",
        [
          QCheck_alcotest.to_alcotest ~speed_level:`Quick
            ~rand:(state_for "safe-legacy-trees") safe_legacy_trees_are_preserved;
        ] );
    ]
