...
[ ! -e "$root/.paengi/demo-v1-git-export" ] || fail 'Git export demonstration was already run for this root'
...
if run_paengi git export release --repository "$root/not-a-git-repository" --release "$release" > "$root/.paengi/demo-v1-git-export-invalid-repository" 2>&1; then
...
git init -q "$git_repository" > "$root/.paengi/demo-v1-git-export-init"
run_paengi git export release --repository "$git_repository" --release "$release" > "$root/.paengi/demo-v1-git-export"
export_output=$(cat "$root/.paengi/demo-v1-git-export")
...
git -C "$git_repository" fsck --full > "$root/.paengi/demo-v1-git-export-fsck"
git -C "$git_repository" rev-parse "$git_ref" > "$root/.paengi/demo-v1-git-export-ref"
[ "$(cat "$root/.paengi/demo-v1-git-export-ref")" = "$git_commit" ] || fail 'Git ref does not name exported commit'
git -C "$git_repository" remote > "$root/.paengi/demo-v1-git-export-remotes"
[ ! -s "$root/.paengi/demo-v1-git-export-remotes" ] || fail 'demo Git repository unexpectedly has a remote'
git -C "$git_repository" checkout -q "$git_commit"
printf '%s\n' 'workspace first capsule bytes' > "$root/.paengi/demo-v1-git-export-todo-oracle"
printf '%s\n' 'workspace second capsule bytes' > "$root/.paengi/demo-v1-git-export-notes-oracle"
cmp -s "$root/.paengi/demo-v1-git-export-todo-oracle" "$git_repository/docs/todo.txt" || fail 'Git export todo bytes differ'
cmp -s "$root/.paengi/demo-v1-git-export-notes-oracle" "$git_repository/notes.txt" || fail 'Git export notes bytes differ'
