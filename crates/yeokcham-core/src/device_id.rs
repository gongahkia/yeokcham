...
/// Opaque identity for an authorised repository device.
///
/// Values are RFC 9562 UUIDv4 identifiers stored as 16 bytes. Device signing
/// keys and authorisation are bound separately. Text input must use the
...
impl DeviceId {
    /// Generates an RFC 9562 UUIDv4 device identity.
    ...
    pub fn from_bytes(bytes: [u8; 16]) -> Result<Self> {
        ...
        if uuid.get_variant() != Variant::RFC4122 || uuid.get_version() != Some(Version::Random) {
            return Err(Error::new(
                ...
                "device ID is not an RFC 9562 UUIDv4",
            ));
        }
        Ok(Self(bytes))
    }
...
impl FromStr for DeviceId {
    ...
    fn from_str(value: &str) -> Result<Self> {
        let uuid = Uuid::parse_str(value).map_err(|source| {
            Error::with_source(ErrorKind::InvalidInput, "device ID is not a UUID", source)
        })?;
        if uuid.hyphenated().to_string() != value {
            return Err(Error::new(
                ErrorKind::InvalidInput,
                "device ID is not in canonical lowercase hyphenated form",
            ));
        }
...
mod tests {
    ...
    fn canonical_text_round_trips() {
        let id: DeviceId = CANONICAL.parse().expect("valid device ID");

        assert_eq!(id.to_string(), CANONICAL);
        assert_eq!(
            DeviceId::from_bytes(id.into_bytes()).expect("valid bytes"),
            id
        );
    ...
    fn rejects_wrong_version_and_variant() {
        let valid: DeviceId = CANONICAL.parse().expect("valid device ID");
        ...
        for bytes in [[0; 16], wrong_version, wrong_variant] {
            let error = DeviceId::from_bytes(bytes).expect_err("invalid UUID must fail");
            assert_eq!(error.kind(), ErrorKind::InvalidInput);
            assert_eq!(
                error.public_message(),
                "device ID is not an RFC 9562 UUIDv4"
    ...
    fn malformed_text_retains_source_without_default_disclosure() {
        ...
        assert_eq!(error.kind(), ErrorKind::InvalidInput);
        assert_eq!(error.to_string(), "device ID is not a UUID");
    ...
    fn debug_is_redacted_and_display_is_explicit() {
        ...
        assert_eq!(format!("{id:?}"), "DeviceId(<redacted>)");
        assert_eq!(format!("{id}"), CANONICAL);
    }

    #[test]
    fn device_id_is_send_and_sync() {
