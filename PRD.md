...
Users keep using ordinary Git. Yeokcham stores the canonical repository on local disk, a self-hosted Yeokcham server, Google Drive, or another backend. It preserves Git object identities while internally representing file content through manifests, chunks, compressed segments, and indexes.
...
Yeokcham should materially reduce storage growth for repositories containing repeated or locally modified large content, while avoiding excessive overhead for tiny source files.
...
The canonical remote must be able to run without GitHub or a Yeokcham-operated service.
...
1. User runs `yeokcham init --from-git .`.
2. Yeokcham scans reachable Git objects.
3. Yeokcham stores metadata objects.
4. Yeokcham selects a storage policy for each blob.
5. Yeokcham writes manifests, chunks, segments, and indexes.
6. Yeokcham verifies that every reachable Git object can be reconstructed.
7. Yeokcham records refs transactionally.

Acceptance condition: a full export produces a repository accepted by `git fsck`.

### J2 — Clone through Yeokcham

1. User runs `git clone yeokcham://path-or-remote/repo`.
2. Git invokes `git-remote-yeokcham`.
3. Yeokcham advertises refs and capabilities.
4. Yeokcham resolves requested objects.
5. Yeokcham reconstructs or synthesises a valid Git pack.
...
### J3 — Push through Yeokcham

1. Git sends updates and a pack.
2. Yeokcham validates the pack and proposed ref changes.
3. Yeokcham ingests objects into its storage representation.
4. Yeokcham writes immutable data first.
5. Yeokcham commits ref updates through a journaled transaction.
6. Yeokcham reports success only after durable state is visible.
...
2. Yeokcham creates or selects a repository folder.
3. Yeokcham uploads encrypted immutable segments and indexes.
4. Yeokcham writes per-device ref journal entries.
...
3. Yeokcham converts or reconstructs standard Git objects.
4. Yeokcham pushes selected refs.
5. Yeokcham records the last mirrored Git state.
6. Yeokcham detects remote changes and imports them.
...
### J6 — Recover without Yeokcham cloud services

1. User obtains the local key material and remote repository folder.
2. User runs `yeokcham recover`.
3. Yeokcham validates segments, indexes, manifests, journals, and signatures.
4. Yeokcham reconstructs refs.
5. Yeokcham exports a conventional Git repository.

Acceptance condition: recovery succeeds using only open-source Yeokcham binaries, repository data, and user-held keys.

## 5. Functional requirements

### FR-001 Repository import

Yeokcham shall import all reachable Git commit, tree, blob, and tag objects.

### FR-002 Object identity preservation

Yeokcham shall preserve the original Git object ID for each imported object.

### FR-003 Conventional export

Yeokcham shall export a repository that passes `git fsck --full`.

### FR-004 Remote helper

Yeokcham shall provide a `git-remote-yeokcham` executable compatible with Git remote-helper invocation.

### FR-005 Clone and fetch

Yeokcham shall support clone and fetch for local Yeokcham stores before network backends are implemented.

### FR-006 Push

Yeokcham shall support push with atomic ref updates.

### FR-007 Adaptive blob representation

Yeokcham shall support at least:
...
Yeokcham shall define a backend interface with operations equivalent to:
...
Yeokcham shall provide a filesystem backend.

### FR-012 Google Drive backend

Yeokcham shall provide a Google Drive backend that stores opaque immutable files rather than a live `.git` directory.

### FR-013 Encryption

Yeokcham shall encrypt repository content and sensitive metadata before remote upload.

### FR-014 Key export and recovery

Yeokcham shall provide explicit backup and restore commands for key material.

### FR-015 Ref journal

Yeokcham shall represent remote mutable state using append-only, verifiable journal entries.

### FR-016 Multi-device reconciliation

Yeokcham shall detect divergent ref updates from different devices and preserve both states for manual or policy-driven resolution.

### FR-017 Partial retrieval

Yeokcham shall support metadata-first retrieval and deferred blob hydration.

### FR-018 Local cache

Yeokcham shall cache reconstructed objects, chunks, and indexes locally with integrity validation.

### FR-019 GitHub mirror

Yeokcham shall push selected standard Git refs to GitHub through ordinary Git transport.

### FR-020 GitHub ingestion

Yeokcham shall be able to fetch and ingest commits created outside Yeokcham on a configured GitHub remote.

### FR-021 Integrity verification

Yeokcham shall expose `yeokcham verify` with fast and full verification modes.

### FR-022 Garbage collection

Yeokcham shall identify unreachable manifests, chunks, and segments subject to retention and backend safety rules.

### FR-023 Repository diagnostics

Yeokcham shall expose storage statistics, cache statistics, backend health, and mirror state.

### FR-024 Self-hosted server

Yeokcham shall provide a single-user HTTP service for repository access and inspection after the local remote is stable.
...
Repository access must not require a Yeokcham-operated account.
...
- A developer can install Yeokcham and create a sovereign remote in under ten minutes.
- An existing repository can be imported without history rewriting.
- A user can export and remove Yeokcham without losing repository history.
...
Mitigation: publish a workload matrix and state where Yeokcham is slower.
