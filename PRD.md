...
Paengi is a local-first version-control system that separates scratch, intent, and release history.
...
Paengi should import Git repositories and export selected release or capsule history to Git.

## 3. Core user journeys

### J1 — Automatic checkpointing

1. User runs `paengi init`.
2. Paengi records a base snapshot.
3. Filesystem changes are observed or scanned.
4. Paengi records scratch events and checkpoints.
...
3. User runs `paengi capsule create`.
4. Paengi proposes file operations and optional semantic operations.
5. User supplies a name and description.
6. Paengi records a stable capsule ID and immutable first revision.
...
3. Paengi records scratch checkpoints.
4. User folds selected work into the capsule.
5. Paengi creates a new capsule revision.
...
3. Paengi orders them using dependencies and explicit precedence.
4. Paengi materialises the workspace.
...
2. User asks Paengi to retarget a capsule.
3. Paengi attempts semantic-anchor replay.
4. Paengi falls back to token or textual context.
5. Paengi records successful operations, uncertain operations, and conflicts.
6. User resolves or accepts the result.
7. Paengi creates a new capsule revision.
...
2. Paengi verifies dependency closure.
3. Paengi materialises the exact snapshot.
4. Configured validation commands run.
5. Paengi records test evidence.
6. Paengi creates an immutable signed release.
...
2. Paengi materialises standard snapshots.
3. Paengi emits one Git commit per selected capsule revision or another explicit export policy.
4. Paengi records paengi-to-Git mapping.
5. User pushes the branch to GitHub with ordinary Git.

Acceptance condition: exported repository passes `git fsck` and matches the Paengi release bytes.

## 4. Functional requirements

### FR-001 Repository initialisation

Paengi shall initialise a repository without requiring Git.

### FR-002 Canonical snapshot store

Paengi shall store exact file bytes, directory structure, executable mode, and supported symlink information.

### FR-003 Automatic scratch history

Paengi shall create scratch checkpoints automatically through scanning first and filesystem events later.

### FR-004 Restore

Paengi shall restore any retained scratch checkpoint.

### FR-005 Scratch timeline

Paengi shall display checkpoint time, changed paths, size, tags, validation state, and retention status.
...
Paengi shall compact unpinned scratch history while preserving all retained checkpoint states.

### FR-008 Capsule creation

Paengi shall create a change capsule from selected scratch history or snapshot differences.
...
Paengi shall deterministically compose selected capsule revisions into a workspace.
...
Paengi shall represent conflicts as persistent repository objects.
...
Paengi shall attempt to apply a capsule revision to a changed base and create a new revision or conflict set.
...
Paengi shall support structured operations for at least one language after the byte model is stable.

### FR-019 Validation evidence

Paengi shall record configured build, test, type-check, or lint results against capsule revisions and releases.

### FR-020 Release creation

Paengi shall create immutable reproducible release snapshots.

### FR-021 Release signatures

Paengi shall support signing releases after the local model is stable.

### FR-022 Git import

Paengi shall import Git commits as opaque or inferred capsules while preserving a mapping.

### FR-023 Git export

Paengi shall export selected paengi history to a valid Git repository.

### FR-024 Storage statistics

Paengi shall report scratch, capsule, release, chunk, and retained-history storage separately.

### FR-025 Integrity verification

Paengi shall verify object hashes, snapshot reachability, capsule dependencies, and release reproducibility.
...
paengi compact --dry-run

paengi capsule create --from <checkpoint-a>..<checkpoint-b>
paengi capsule list
paengi capsule show <capsule>
paengi capsule revise <capsule>
paengi capsule split <capsule>
paengi capsule combine <capsule-a> <capsule-b>
paengi capsule retarget <capsule> --onto <release>

paengi work base <release>
paengi work enable <capsule>
paengi work disable <capsule>
paengi work list
paengi work materialise

paengi conflict list
paengi conflict show <conflict>
paengi conflict resolve <conflict>

paengi release create
paengi release verify <release>

paengi import git <path>
paengi export git <destination>
paengi verify
...
- Reproducing every Git topology exactly as native Paengi concepts.
