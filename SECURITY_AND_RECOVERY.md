...
`yeokcham-server` V1 is a read-only native HTTP service, not Git smart HTTP. It serves verified ref metadata and exact Git object bodies only through loopback TCP. Its parser accepts one ASCII `GET` request without a body per connection, limits headers to 8 KiB, response bodies to 64 MiB, I/O phases to five seconds, active workers to four, and queued sockets to eight. The CLI and server API reject every non-loopback address, so V1 cannot accidentally bind `0.0.0.0` or a LAN interface. This is not authentication: any local process able to connect to the host's loopback interface may read the served repository's refs and objects. V1 must not be exposed through a proxy, tunnel, or port forward. Authentication, TLS termination, and any non-loopback deployment remain unavailable until the single-user authentication design is implemented. Responses and default errors omit repository paths, storage details, and source bytes from logs; server termination cannot mutate canonical data. The wire contract is [`docs/native-http-v1.md`](docs/native-http-v1.md).
